# Stage A Conversion 05 — `app/modules/ui/skeletons.js` IIFE → ESM
...
- **Module:** `app/modules/ui/skeletons.js`
- **CL namespace:** `ui/skeletons`
- **Baseline size (IIFE era):** 86 lines (re-snapshot before editing)
...
- [ ] `app/modules/ui/skeletons.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/ui/skeletons.js` passes (pre-state).
- [ ] `wc -l app/modules/ui/skeletons.js` snapshotted (expect ~86).
...
3. `export` every member previously assigned to `CL.ui.*` or
...
4. Keep transitional `CL.ui = CL.ui || {}; CL.ui.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('ui/skeletons');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/ui/skeletons.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/ui/skeletons.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: ui/skeletons`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/ui/skeletons.js
```
