# Stage A Conversion 32 — `app/modules/assets/asset-export.js` IIFE → ESM
...
- **Module:** `app/modules/assets/asset-export.js`
- **CL namespace:** `assets/asset-export`
- **Baseline size (IIFE era):** 459 lines (re-snapshot before editing)
...
- [ ] `app/modules/assets/asset-export.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/assets/asset-export.js` passes (pre-state).
- [ ] `wc -l app/modules/assets/asset-export.js` snapshotted (expect ~459).
...
3. `export` every member previously assigned to `CL.assets.*` or
...
4. Keep transitional `CL.assets = CL.assets || {}; CL.assets.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('assets/asset-export');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/assets/asset-export.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/assets/asset-export.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: assets/asset-export`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/assets/asset-export.js
```
