# CC Modular Extraction Prompt 34 — `app/modules/map/map-safe-helpers.js`
...
# 1. Locate the block. Snapshot range: L19966-L20032 (feature: Safe Leaflet fly/fit wrappers.)
grep -nE 'function +safeFlyTo\b|const +safeFlyTo\b|let +safeFlyTo\b|window\.safeFlyTo\b|safeFlyTo *= *function|safeFlyTo *= *async|safeFlyTo *= *\(|function +safeFitBounds\b|const +safeFitBounds\b|let +safeFitBounds\b|window\.safeFitBounds\b|safeFitBounds *= *function|safeFitBounds *= *async|safeFitBounds *= *\(|function +safeFlyToBounds\b|const +safeFlyToBounds\b|let +safeFlyToBounds\b|window\.safeFlyToBounds\b|safeFlyToBounds *= *function|safeFlyToBounds *= *async|safeFlyToBounds *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=19966 && ($2+0)<=20032' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/map/map-safe-helpers.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/spatial/r2-resolve.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L19966–L20032, ~67 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `safeFlyTo` (and the helpers between it and the next named decl)
- `safeFitBounds` (and the helpers between it and the next named decl)
- `safeFlyToBounds` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/map/map-safe-helpers.js`:

```js
/**
 * CL map.safe module
 *
 * Extracted from app/index.html (snapshot L19966-L20032) on 2026-05-15.
 ...
 *   - window.safeFlyTo → CL.map.safe.safeFlyTo
 *   - window.safeFitBounds → CL.map.safe.safeFitBounds
 *   - window.safeFlyToBounds → CL.map.safe.safeFlyToBounds
 *
...
```
Registration string is **path-style** `'map/map-safe-helpers'` (matches `loader.js`
convention — logs `[CL] Module loaded: map/map-safe-helpers`). If `CL.map.safe` needs a new
...
`<script src="modules/spatial/r2-resolve.js"></script>` (this places it in the **EARLY** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/map/map-safe-helpers.js"></script>
```
...
wc -l app/modules/map/map-safe-helpers.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +safeFlyTo\b|const +safeFlyTo\b|let +safeFlyTo\b|window\.safeFlyTo\b|safeFlyTo *= *function|safeFlyTo *= *async|safeFlyTo *= *\(|function +safeFitBounds\b|const +safeFitBounds\b|let +safeFitBounds\b|window\.safeFitBounds\b|safeFitBounds *= *function|safeFitBounds *= *async|safeFitBounds *= *\(|function +safeFlyToBounds\b|const +safeFlyToBounds\b|let +safeFlyToBounds\b|window\.safeFlyToBounds\b|safeFlyToBounds *= *function|safeFlyToBounds *= *async|safeFlyToBounds *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +safeFlyTo\b|const +safeFlyTo\b|let +safeFlyTo\b|window\.safeFlyTo\b|safeFlyTo *= *function|safeFlyTo *= *async|safeFlyTo *= *\(|function +safeFitBounds\b|const +safeFitBounds\b|let +safeFitBounds\b|window\.safeFitBounds\b|safeFitBounds *= *function|safeFitBounds *= *async|safeFitBounds *= *\(|function +safeFlyToBounds\b|const +safeFlyToBounds\b|let +safeFlyToBounds\b|window\.safeFlyToBounds\b|safeFlyToBounds *= *function|safeFlyToBounds *= *async|safeFlyToBounds *= *\(' app/modules/map/map-safe-helpers.js       # expected: the anchors present
grep -c '<script src="modules/map/map-safe-helpers.js"></script>' app/index.html  # 1
node --check app/modules/map/map-safe-helpers.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: map/map-safe-helpers`.
...
```
- Exercise the feature these functions drive: Safe Leaflet fly/fit wrappers.
- In DevTools confirm `typeof window.safeFlyTo` → `'function'` and
  `typeof CL.map.safe` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/map/map-safe-helpers.js
git checkout -- app/index.html
rm app/modules/map/map-safe-helpers.js
```
