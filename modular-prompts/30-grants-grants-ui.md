# CC Modular Extraction Prompt 30 — `app/modules/grants/grants-ui.js`
...
test -f app/modules/grants/grants-ui.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/grants/grants-email.js"></script>' app/index.html   # expected: 1 match
```
...
## §2 Where to put it
Create `app/modules/grants/grants-ui.js`:
...
```
Registration string is **path-style** `'grants/grants-ui'` (matches `loader.js`
convention — logs `[CL] Module loaded: grants/grants-ui`). If `CL.grants.ui` needs a new
...
`<script src="modules/grants/grants-email.js"></script>` (this places it in the **LATE** cluster
...
```
On app load the console must show: `[CL] Module loaded: grants/grants-ui`.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/grants/grants-ui.js
git checkout -- app/index.html
rm app/modules/grants/grants-ui.js
```
