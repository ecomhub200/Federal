# CC Modular Extraction Prompt 35 — `app/modules/map/map-init.js`
...
# 1. Locate the block. Snapshot range: L51400-L53500 (feature: Leaflet map init + base layers + viewport.)
grep -nE 'function +initMap\b|const +initMap\b|let +initMap\b|window\.initMap\b|initMap *= *function|initMap *= *async|initMap *= *\(|function +updateMapDisplay\b|const +updateMapDisplay\b|let +updateMapDisplay\b|window\.updateMapDisplay\b|updateMapDisplay *= *function|updateMapDisplay *= *async|updateMapDisplay *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=51400 && ($2+0)<=53500' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/map/map-init.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/cmf/cmf-deficiency.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L51400–L53500, ~2101 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `initMap` (and the helpers between it and the next named decl)
- `updateMapDisplay` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/map/map-init.js`:

```js
/**
 * CL map.init module
 *
 * Extracted from app/index.html (snapshot L51400-L53500) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/35-map-map-init.md.
 * Responsibility: Leaflet map init + base layers + viewport.
 *
 * Public API (back-compat dual exposure):
 *   - window.initMap → CL.map.init.initMap
 *   - window.updateMapDisplay → CL.map.init.updateMapDisplay
 *
 * Depends on (must load before this file): `map/map-safe-helpers`, `spatial/spatial-clip`
 */
(function(){
  'use strict';
  ...
  window.CL = window.CL || {};
  CL.map = CL.map || {};
  window.initMap = initMap; CL.map.initMap = initMap;
  window.updateMapDisplay = updateMapDisplay; CL.map.updateMapDisplay = updateMapDisplay;
  CL._registerModule('map/map-init');
})();
```
Registration string is **path-style** `'map/map-init'` (matches `loader.js`
convention — logs `[CL] Module loaded: map/map-init`). If `CL.map.init` needs a new
...
`<script src="modules/cmf/cmf-deficiency.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/map/map-init.js"></script>
```
...
wc -l app/modules/map/map-init.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initMap\b|const +initMap\b|let +initMap\b|window\.initMap\b|initMap *= *function|initMap *= *async|initMap *= *\(|function +updateMapDisplay\b|const +updateMapDisplay\b|let +updateMapDisplay\b|window\.updateMapDisplay\b|updateMapDisplay *= *function|updateMapDisplay *= *async|updateMapDisplay *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initMap\b|const +initMap\b|let +initMap\b|window\.initMap\b|initMap *= *function|initMap *= *async|initMap *= *\(|function +updateMapDisplay\b|const +updateMapDisplay\b|let +updateMapDisplay\b|window\.updateMapDisplay\b|updateMapDisplay *= *function|updateMapDisplay *= *async|updateMapDisplay *= *\(' app/modules/map/map-init.js       # expected: the anchors present
grep -c '<script src="modules/map/map-init.js"></script>' app/index.html  # 1
node --check app/modules/map/map-init.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: map/map-init`.
...
```
- Exercise the feature these functions drive: Leaflet map init + base layers + viewport.
- In DevTools confirm `typeof window.initMap` → `'function'` and
  `typeof CL.map.init` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/map/map-init.js
git checkout -- app/index.html
rm app/modules/map/map-init.js
```
