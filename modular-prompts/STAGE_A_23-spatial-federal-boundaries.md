# Stage A Conversion 23 — `app/modules/spatial/federal-boundaries.js` IIFE → ESM
...
- **Module:** `app/modules/spatial/federal-boundaries.js`
- **CL namespace:** `spatial/federal-boundaries`
- **Baseline size (IIFE era):** 204 lines (re-snapshot before editing)
...
- [ ] `app/modules/spatial/federal-boundaries.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/spatial/federal-boundaries.js` passes (pre-state).
- [ ] `wc -l app/modules/spatial/federal-boundaries.js` snapshotted (expect ~204).
...
5. Keep `CL._registerModule('spatial/federal-boundaries');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/spatial/federal-boundaries.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/spatial/federal-boundaries.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: spatial/federal-boundaries`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/spatial/federal-boundaries.js
```
