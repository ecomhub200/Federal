# Stage A Conversion 28 — `app/modules/upload/api-connector.js` IIFE → ESM
...
- **Module:** `app/modules/upload/api-connector.js`
- **CL namespace:** `upload/api-connector`
- **Baseline size (IIFE era):** 333 lines (re-snapshot before editing)
...
- [ ] `app/modules/upload/api-connector.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/upload/api-connector.js` passes (pre-state).
- [ ] `wc -l app/modules/upload/api-connector.js` snapshotted (expect ~333).
...
5. Keep `CL._registerModule('upload/api-connector');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/upload/api-connector.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/upload/api-connector.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: upload/api-connector`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/upload/api-connector.js
```
