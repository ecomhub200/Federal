# Stage A Conversion 43 — `app/modules/batch-ba/batch-ba-upload.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-upload.js`
- **CL namespace:** `batch-ba/upload`
- **Baseline size (IIFE era):** 380 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-upload.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-upload.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-upload.js` snapshotted (expect ~380).
...
5. Keep `CL._registerModule('batch-ba/upload');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-upload.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-upload.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/upload`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-upload.js
```
