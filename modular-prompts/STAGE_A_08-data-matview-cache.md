# Stage A Conversion 08 — `app/modules/data/matview-cache.js` IIFE → ESM
...
- **Module:** `app/modules/data/matview-cache.js`
- **CL namespace:** `data/matview-cache`
- **Baseline size (IIFE era):** 121 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/matview-cache.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/matview-cache.js` passes (pre-state).
- [ ] `wc -l app/modules/data/matview-cache.js` snapshotted (expect ~121).
...
5. Keep `CL._registerModule('data/matview-cache');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/matview-cache.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/matview-cache.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/matview-cache`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/matview-cache.js
```
