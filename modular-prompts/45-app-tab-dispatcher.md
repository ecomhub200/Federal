# CC Modular Extraction Prompt 45 — `app/modules/app/tab-dispatcher.js`
...
# 1. Locate the block. Snapshot range: L31759-L32200 (feature: showTab()/navigateTo() central tab dispatcher.)
grep -nE 'function +showTab\b|const +showTab\b|let +showTab\b|window\.showTab\b|showTab *= *function|showTab *= *async|showTab *= *\(|function +navigateTo\b|const +navigateTo\b|let +navigateTo\b|window\.navigateTo\b|navigateTo *= *function|navigateTo *= *async|navigateTo *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=31759 && ($2+0)<=32200' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/app/tab-dispatcher.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/data/filter-wiring.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L31759–L32200, ~442 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `showTab` (and the helpers between it and the next named decl)
- `navigateTo` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/app/tab-dispatcher.js`:
...
```
Registration string is **path-style** `'app/tab-dispatcher'` (matches `loader.js`
convention — logs `[CL] Module loaded: app/tab-dispatcher`). If `CL.app.tabDispatcher` needs a new
...
`<script src="modules/data/filter-wiring.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/app/tab-dispatcher.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +showTab\b|const +showTab\b|let +showTab\b|window\.showTab\b|showTab *= *function|showTab *= *async|showTab *= *\(|function +navigateTo\b|const +navigateTo\b|let +navigateTo\b|window\.navigateTo\b|navigateTo *= *function|navigateTo *= *async|navigateTo *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +showTab\b|const +showTab\b|let +showTab\b|window\.showTab\b|showTab *= *function|showTab *= *async|showTab *= *\(|function +navigateTo\b|const +navigateTo\b|let +navigateTo\b|window\.navigateTo\b|navigateTo *= *function|navigateTo *= *async|navigateTo *= *\(' app/modules/app/tab-dispatcher.js       # expected: the anchors present
grep -c '<script src="modules/app/tab-dispatcher.js"></script>' app/index.html  # 1
node --check app/modules/app/tab-dispatcher.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: app/tab-dispatcher`.
...
```
- Exercise the feature these functions drive: showTab()/navigateTo() central tab dispatcher.
- In DevTools confirm `typeof window.showTab` → `'function'` and
  `typeof CL.app.tabDispatcher` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/app/tab-dispatcher.js
git checkout -- app/index.html
rm app/modules/app/tab-dispatcher.js
```
