# Stage A Conversion 47 — `app/modules/batch-ba/batch-ba-export-csv.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-export-csv.js`
- **CL namespace:** `batch-ba/export-csv`
- **Baseline size (IIFE era):** 82 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-export-csv.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-export-csv.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-export-csv.js` snapshotted (expect ~82).
...
5. Keep `CL._registerModule('batch-ba/export-csv');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-export-csv.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-export-csv.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/export-csv`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-export-csv.js
```
