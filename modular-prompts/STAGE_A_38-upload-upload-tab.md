# Stage A Conversion 38 — `app/modules/upload/upload-tab.js` IIFE → ESM
...
- **Module:** `app/modules/upload/upload-tab.js`
- **CL namespace:** `upload/upload-tab`
- **Baseline size (IIFE era):** 799 lines (re-snapshot before editing)
...
- [ ] `app/modules/upload/upload-tab.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/upload/upload-tab.js` passes (pre-state).
- [ ] `wc -l app/modules/upload/upload-tab.js` snapshotted (expect ~799).
...
import { /* roadTypeMapping */ } from '../data/road-type-mapping.js';
```
...
5. Keep `CL._registerModule('upload/upload-tab');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/upload/upload-tab.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/upload/upload-tab.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: upload/upload-tab`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/upload/upload-tab.js
```
