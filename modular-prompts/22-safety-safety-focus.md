# CC Modular Extraction Prompt 22 — `app/modules/safety/safety-focus.js`
...
# 1. Locate the block. Snapshot range: L99600-L105299 (feature: Safety Focus tab: category selection, capability gates.)
grep -nE 'function +initSafetyFocus\b|const +initSafetyFocus\b|let +initSafetyFocus\b|window\.initSafetyFocus\b|initSafetyFocus *= *function|initSafetyFocus *= *async|initSafetyFocus *= *\(|function +updateSafetyCards\b|const +updateSafetyCards\b|let +updateSafetyCards\b|window\.updateSafetyCards\b|updateSafetyCards *= *function|updateSafetyCards *= *async|updateSafetyCards *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=99600 && ($2+0)<=105299' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/safety/safety-focus.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/fatal-speeding/fatal-speeding-tab.js"></script>' app/index.html   # expected: 1 match
```
...
- `initSafetyFocus` (and the helpers between it and the next named decl)
- `updateSafetyCards` (and the helpers between it and the next named decl)
...
```js
/**
 * CL safety.focus module
 *
 * Extracted from app/index.html (snapshot L99600-L105299) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/22-safety-safety-focus.md.
...
(function(){
  ...
  window.CL = window.CL || {};
  CL.safety = CL.safety || {};
  window.initSafetyFocus = initSafetyFocus; CL.safety.initSafetyFocus = initSafetyFocus;
  window.updateSafetyCards = updateSafetyCards; CL.safety.updateSafetyCards = updateSafetyCards;
  CL._registerModule('safety/safety-focus');
})();
```
Registration string is **path-style** `'safety/safety-focus'` (matches `loader.js`
convention — logs `[CL] Module loaded: safety/safety-focus`). If `CL.safety.focus` needs a new
...
`<script src="modules/fatal-speeding/fatal-speeding-tab.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/safety/safety-focus.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initSafetyFocus\b|const +initSafetyFocus\b|let +initSafetyFocus\b|window\.initSafetyFocus\b|initSafetyFocus *= *function|initSafetyFocus *= *async|initSafetyFocus *= *\(|function +updateSafetyCards\b|const +updateSafetyCards\b|let +updateSafetyCards\b|window\.updateSafetyCards\b|updateSafetyCards *= *function|updateSafetyCards *= *async|updateSafetyCards *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initSafetyFocus\b|const +initSafetyFocus\b|let +initSafetyFocus\b|window\.initSafetyFocus\b|initSafetyFocus *= *function|initSafetyFocus *= *async|initSafetyFocus *= *\(|function +updateSafetyCards\b|const +updateSafetyCards\b|let +updateSafetyCards\b|window\.updateSafetyCards\b|updateSafetyCards *= *function|updateSafetyCards *= *async|updateSafetyCards *= *\(' app/modules/safety/safety-focus.js       # expected: the anchors present
grep -c '<script src="modules/safety/safety-focus.js"></script>' app/index.html  # 1
node --check app/modules/safety/safety-focus.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: safety/safety-focus`.
...
```
- Exercise the feature these functions drive: Safety Focus tab: category selection, capability gates.
- In DevTools confirm `typeof window.initSafetyFocus` → `'function'` and
  `typeof CL.safety.focus` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/safety/safety-focus.js
git checkout -- app/index.html
rm app/modules/safety/safety-focus.js
```
