# Stage A Conversion 35 — `app/modules/worker/sample-rows-loader.js` IIFE → ESM
...
- **Module:** `app/modules/worker/sample-rows-loader.js`
- **CL namespace:** `worker/sample-rows-loader`
- **Baseline size (IIFE era):** 198 lines (re-snapshot before editing)
...
- [ ] `app/modules/worker/sample-rows-loader.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/worker/sample-rows-loader.js` passes (pre-state).
- [ ] `wc -l app/modules/worker/sample-rows-loader.js` snapshotted (expect ~198).
...
3. `export` every member previously assigned to `CL.worker.*` or
...
4. Keep transitional `CL.worker = CL.worker || {}; CL.worker.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('worker/sample-rows-loader');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/worker/sample-rows-loader.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/worker/sample-rows-loader.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: worker/sample-rows-loader`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/worker/sample-rows-loader.js
```
