# Stage A Conversion 27 — `app/modules/map/map-safe-helpers.js` IIFE → ESM
...
- **Module:** `app/modules/map/map-safe-helpers.js`
- **CL namespace:** `map/map-safe-helpers`
- **Baseline size (IIFE era):** 95 lines (re-snapshot before editing)
...
- [ ] `app/modules/map/map-safe-helpers.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/map/map-safe-helpers.js` passes (pre-state).
- [ ] `wc -l app/modules/map/map-safe-helpers.js` snapshotted (expect ~95).
...
5. Keep `CL._registerModule('map/map-safe-helpers');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/map/map-safe-helpers.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/map/map-safe-helpers.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: map/map-safe-helpers`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/map/map-safe-helpers.js
```
