# Stage A Conversion 59 — `app/modules/reports/reports-standard-types2.js` IIFE → ESM
...
- **Module:** `app/modules/reports/reports-standard-types2.js`
- **CL namespace:** `reports/reports-standard-types2` (CL key `CL.reports.standardTypes`)
- **Baseline size (IIFE era):** 437 lines (re-snapshot before editing)
- **Post-snapshot module** — depends on `reports-standard-types.js`
  (one-directional, acyclic). Folded into the Session K design docs.
...
- [ ] `app/modules/reports/reports-standard-types2.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/reports/reports-standard-types2.js` passes (pre-state).
- [ ] `wc -l app/modules/reports/reports-standard-types2.js` snapshotted (expect ~437).
- [ ] `STAGE_A_58` (reports-standard-types) is applied on this branch first
      (this file imports from it).
...
3. `export` every member previously assigned to `CL.reports.standardTypes.*`
   or `window.*` (public API). Leave module-private helpers unexported.

### Imports to ADD (top of file)
One real static edge (was `CL.reports.standardTypes.*` reads owned by the
`-types` base file):
```js
import { /* members read */ } from './reports-standard-types.js';  // was CL.reports.standardTypes.*
```
**TDZ caution:** only reference these imported bindings inside function
bodies called at runtime — never at module top-level evaluation.

### `window.*` policy (see STAGE_A_ONCLICK_API.md)
This module owns **no** onclick survivor → **delete ALL `window.*`
lines**. Consumers `import { … }` the members instead. If the post-01–46
re-scan finds an inline-`index.html` reader, add it to the legacy block
then (FLOOR — extend, never shrink).
...
5. Keep `CL._registerModule('reports/reports-standard-types2');` (load tracker).
...
- Add this module to `app/main.js` at its topo position (L2 — one-hop on
  `reports-standard-types`; see `STAGE_A_MAIN_ENTRY_DRAFT.js`, already
  listed). Do **not** touch `app/index.html` script tags in this prompt.

## §3 Post-flight
- [ ] `node --check app/modules/reports/reports-standard-types2.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/reports/reports-standard-types2.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: reports/reports-standard-types2`
      prints; the Standard Reports band renders.

## §4 Rollback
```bash
git checkout -- app/modules/reports/reports-standard-types2.js
```
