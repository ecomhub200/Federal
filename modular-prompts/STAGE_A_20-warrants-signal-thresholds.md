# Stage A Conversion 20 — `app/modules/warrants/signal-thresholds.js` IIFE → ESM
...
- **Module:** `app/modules/warrants/signal-thresholds.js`
- **CL namespace:** `warrants/signal-thresholds`
- **Baseline size (IIFE era):** 105 lines (re-snapshot before editing)
...
- [ ] `app/modules/warrants/signal-thresholds.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/warrants/signal-thresholds.js` passes (pre-state).
- [ ] `wc -l app/modules/warrants/signal-thresholds.js` snapshotted (expect ~105).
...
5. Keep `CL._registerModule('warrants/signal-thresholds');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/warrants/signal-thresholds.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/warrants/signal-thresholds.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: warrants/signal-thresholds`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/warrants/signal-thresholds.js
```
