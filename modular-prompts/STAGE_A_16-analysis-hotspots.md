# Stage A Conversion 16 — `app/modules/analysis/hotspots.js` IIFE → ESM
...
- **Module:** `app/modules/analysis/hotspots.js`
- **CL namespace:** `analysis/hotspots`
- **Baseline size (IIFE era):** 59 lines (re-snapshot before editing)
...
- [ ] `app/modules/analysis/hotspots.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/analysis/hotspots.js` passes (pre-state).
- [ ] `wc -l app/modules/analysis/hotspots.js` snapshotted (expect ~59).
...
5. Keep `CL._registerModule('analysis/hotspots');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/analysis/hotspots.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/analysis/hotspots.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: analysis/hotspots`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/analysis/hotspots.js
```
