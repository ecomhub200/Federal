# Stage A Conversion 18 — `app/modules/warrants/signal.js` IIFE → ESM
...
- **Module:** `app/modules/warrants/signal.js`
- **CL namespace:** `warrants/signal`
- **Baseline size (IIFE era):** 85 lines (re-snapshot before editing)
...
- [ ] `app/modules/warrants/signal.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/warrants/signal.js` passes (pre-state).
- [ ] `wc -l app/modules/warrants/signal.js` snapshotted (expect ~85).
...
5. Keep `CL._registerModule('warrants/signal');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/warrants/signal.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/warrants/signal.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: warrants/signal`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/warrants/signal.js
```
