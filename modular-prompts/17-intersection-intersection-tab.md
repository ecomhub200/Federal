# CC Modular Extraction Prompt 17 — `app/modules/intersection/intersection-tab.js`
...
# 1. Locate the block. Snapshot range: L64600-L65800 (feature: Intersections tab + matview integration + detail charts.)
grep -nE 'function +updateIntersectionTab\b|const +updateIntersectionTab\b|let +updateIntersectionTab\b|window\.updateIntersectionTab\b|updateIntersectionTab *= *function|updateIntersectionTab *= *async|updateIntersectionTab *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=64600 && ($2+0)<=65800' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/intersection/intersection-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/hotspots/hotspots-tab.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L64600–L65800, ~1201 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `updateIntersectionTab` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/intersection/intersection-tab.js`:

```js
/**
 * CL intersection.tab module
 *
 * Extracted from app/index.html (snapshot L64600-L65800) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/17-intersection-intersection-tab.md.
 * Responsibility: Intersections tab + matview integration + detail charts.
 *
 * Public API (back-compat dual exposure):
 *   - window.updateIntersectionTab → CL.intersection.tab.updateIntersectionTab
 *
 * Depends on (must load before this file): `spatial/aggregate-loader`, `data/matview-cache`
 */
(function(){
  'use strict';
  ...
  window.updateIntersectionTab = updateIntersectionTab; CL.intersection.updateIntersectionTab = updateIntersectionTab;
  CL._registerModule('intersection/intersection-tab');
})();
```
Registration string is **path-style** `'intersection/intersection-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: intersection/intersection-tab`). If `CL.intersection.tab` needs a new
...
`<script src="modules/hotspots/hotspots-tab.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/intersection/intersection-tab.js"></script>
```
...
wc -l app/modules/intersection/intersection-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +updateIntersectionTab\b|const +updateIntersectionTab\b|let +updateIntersectionTab\b|window\.updateIntersectionTab\b|updateIntersectionTab *= *function|updateIntersectionTab *= *async|updateIntersectionTab *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +updateIntersectionTab\b|const +updateIntersectionTab\b|let +updateIntersectionTab\b|window\.updateIntersectionTab\b|updateIntersectionTab *= *function|updateIntersectionTab *= *async|updateIntersectionTab *= *\(' app/modules/intersection/intersection-tab.js       # expected: the anchors present
grep -c '<script src="modules/intersection/intersection-tab.js"></script>' app/index.html  # 1
node --check app/modules/intersection/intersection-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: intersection/intersection-tab`.
...
```
- Exercise the feature these functions drive: Intersections tab + matview integration + detail charts.
- In DevTools confirm `typeof window.updateIntersectionTab` → `'function'` and
  `typeof CL.intersection.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/intersection/intersection-tab.js
git checkout -- app/index.html
rm app/modules/intersection/intersection-tab.js
```
