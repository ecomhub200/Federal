# Stage A Conversion 15 — `app/modules/analysis/baselines.js` IIFE → ESM
...
- **Module:** `app/modules/analysis/baselines.js`
- **CL namespace:** `analysis/baselines`
- **Baseline size (IIFE era):** 262 lines (re-snapshot before editing)
...
- [ ] `app/modules/analysis/baselines.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/analysis/baselines.js` passes (pre-state).
- [ ] `wc -l app/modules/analysis/baselines.js` snapshotted (expect ~262).
...
5. Keep `CL._registerModule('analysis/baselines');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/analysis/baselines.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/analysis/baselines.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: analysis/baselines`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/analysis/baselines.js
```
