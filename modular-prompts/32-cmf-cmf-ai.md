# CC Modular Extraction Prompt 32 — `app/modules/cmf/cmf-ai.js`
...
test -f app/modules/cmf/cmf-ai.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/cmf/cmf-search.js"></script>' app/index.html   # expected: 1 match
```
...
## §2 Where to put it
Create `app/modules/cmf/cmf-ai.js`:
 ...
 *   - window.initCMFAI → CL.cmf.ai.initCMFAI
 *
 * Depends on (must load before this file): `cmf/cmf-search`, `ai/ai-mode`
 */
(function(){
  'use strict';
...
```
Registration string is **path-style** `'cmf/cmf-ai'` (matches `loader.js`
convention — logs `[CL] Module loaded: cmf/cmf-ai`). If `CL.cmf.ai` needs a new
...
`<script src="modules/cmf/cmf-search.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/cmf/cmf-ai.js"></script>
```
...
wc -l app/modules/cmf/cmf-ai.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initCMFAI\b|const +initCMFAI\b|let +initCMFAI\b|window\.initCMFAI\b|initCMFAI *= *function|initCMFAI *= *async|initCMFAI *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initCMFAI\b|const +initCMFAI\b|let +initCMFAI\b|window\.initCMFAI\b|initCMFAI *= *function|initCMFAI *= *async|initCMFAI *= *\(' app/modules/cmf/cmf-ai.js       # expected: the anchors present
grep -c '<script src="modules/cmf/cmf-ai.js"></script>' app/index.html  # 1
node --check app/modules/cmf/cmf-ai.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: cmf/cmf-ai`.
...
```
- Exercise the feature these functions drive: CMF AI agents + recommendation narrative.
- In DevTools confirm `typeof window.initCMFAI` → `'function'` and
  `typeof CL.cmf.ai` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/cmf/cmf-ai.js
git checkout -- app/index.html
rm app/modules/cmf/cmf-ai.js
```
