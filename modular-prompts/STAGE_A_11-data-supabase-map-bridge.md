# Stage A Conversion 11 — `app/modules/data/supabase-map-bridge.js` IIFE → ESM
...
- **Module:** `app/modules/data/supabase-map-bridge.js`
- **CL namespace:** `data/mapBridge`
- **Baseline size (IIFE era):** 485 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/supabase-map-bridge.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/supabase-map-bridge.js` passes (pre-state).
- [ ] `wc -l app/modules/data/supabase-map-bridge.js` snapshotted (expect ~485).
...
5. Keep `CL._registerModule('data/mapBridge');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/supabase-map-bridge.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/supabase-map-bridge.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/mapBridge`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/supabase-map-bridge.js
```
