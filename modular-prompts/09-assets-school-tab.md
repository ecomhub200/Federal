# CC Modular Extraction Prompt 09 — `app/modules/assets/school-tab.js`
...
# 1. Locate the block. Snapshot range: L154035-L154515 (feature: School-zone safety tab (schoolTab* fns + escapeXML/KML export).)
grep -nE 'function +updateSchoolTabTable\b|const +updateSchoolTabTable\b|let +updateSchoolTabTable\b|window\.updateSchoolTabTable\b|updateSchoolTabTable *= *function|updateSchoolTabTable *= *async|updateSchoolTabTable *= *\(|function +schoolTabExportData\b|const +schoolTabExportData\b|let +schoolTabExportData\b|window\.schoolTabExportData\b|schoolTabExportData *= *function|schoolTabExportData *= *async|schoolTabExportData *= *\(|function +schoolTabExportKML\b|const +schoolTabExportKML\b|let +schoolTabExportKML\b|window\.schoolTabExportKML\b|schoolTabExportKML *= *function|schoolTabExportKML *= *async|schoolTabExportKML *= *\(|function +escapeXML\b|const +escapeXML\b|let +escapeXML\b|window\.escapeXML\b|escapeXML *= *function|escapeXML *= *async|escapeXML *= *\(|function +softActivateSchoolLayer\b|const +softActivateSchoolLayer\b|let +softActivateSchoolLayer\b|window\.softActivateSchoolLayer\b|softActivateSchoolLayer *= *function|softActivateSchoolLayer *= *async|softActivateSchoolLayer *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=154035 && ($2+0)<=154515' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/assets/school-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/safety/sign-deficiency.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L154035–L154515, ~481 lines). The exact
declarations are the `INDEX_MAP_part4.md` rows inside that range. Anchor
declarations (use to find the block):
- `updateSchoolTabTable` (and the helpers between it and the next named decl)
- `schoolTabExportData` (and the helpers between it and the next named decl)
- `schoolTabExportKML` (and the helpers between it and the next named decl)
- `escapeXML` (and the helpers between it and the next named decl)
- `softActivateSchoolLayer` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/assets/school-tab.js`:
 ...
 *   - window.updateSchoolTabTable → CL.assets.schoolTab.updateSchoolTabTable
 *   - window.schoolTabExportData → CL.assets.schoolTab.schoolTabExportData
 *   - window.schoolTabExportKML → CL.assets.schoolTab.schoolTabExportKML
 *   - window.escapeXML → CL.assets.schoolTab.escapeXML
 *   - window.softActivateSchoolLayer → CL.assets.schoolTab.softActivateSchoolLayer
 *
...
```
Registration string is **path-style** `'assets/school-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: assets/school-tab`). If `CL.assets.schoolTab` needs a new
...
`<script src="modules/safety/sign-deficiency.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/assets/school-tab.js"></script>
```
...
wc -l app/modules/assets/school-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +updateSchoolTabTable\b|const +updateSchoolTabTable\b|let +updateSchoolTabTable\b|window\.updateSchoolTabTable\b|updateSchoolTabTable *= *function|updateSchoolTabTable *= *async|updateSchoolTabTable *= *\(|function +schoolTabExportData\b|const +schoolTabExportData\b|let +schoolTabExportData\b|window\.schoolTabExportData\b|schoolTabExportData *= *function|schoolTabExportData *= *async|schoolTabExportData *= *\(|function +schoolTabExportKML\b|const +schoolTabExportKML\b|let +schoolTabExportKML\b|window\.schoolTabExportKML\b|schoolTabExportKML *= *function|schoolTabExportKML *= *async|schoolTabExportKML *= *\(|function +escapeXML\b|const +escapeXML\b|let +escapeXML\b|window\.escapeXML\b|escapeXML *= *function|escapeXML *= *async|escapeXML *= *\(|function +softActivateSchoolLayer\b|const +softActivateSchoolLayer\b|let +softActivateSchoolLayer\b|window\.softActivateSchoolLayer\b|softActivateSchoolLayer *= *function|softActivateSchoolLayer *= *async|softActivateSchoolLayer *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +updateSchoolTabTable\b|const +updateSchoolTabTable\b|let +updateSchoolTabTable\b|window\.updateSchoolTabTable\b|updateSchoolTabTable *= *function|updateSchoolTabTable *= *async|updateSchoolTabTable *= *\(|function +schoolTabExportData\b|const +schoolTabExportData\b|let +schoolTabExportData\b|window\.schoolTabExportData\b|schoolTabExportData *= *function|schoolTabExportData *= *async|schoolTabExportData *= *\(|function +schoolTabExportKML\b|const +schoolTabExportKML\b|let +schoolTabExportKML\b|window\.schoolTabExportKML\b|schoolTabExportKML *= *function|schoolTabExportKML *= *async|schoolTabExportKML *= *\(|function +escapeXML\b|const +escapeXML\b|let +escapeXML\b|window\.escapeXML\b|escapeXML *= *function|escapeXML *= *async|escapeXML *= *\(|function +softActivateSchoolLayer\b|const +softActivateSchoolLayer\b|let +softActivateSchoolLayer\b|window\.softActivateSchoolLayer\b|softActivateSchoolLayer *= *function|softActivateSchoolLayer *= *async|softActivateSchoolLayer *= *\(' app/modules/assets/school-tab.js       # expected: the anchors present
grep -c '<script src="modules/assets/school-tab.js"></script>' app/index.html  # 1
node --check app/modules/assets/school-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: assets/school-tab`.
...
```
- Exercise the feature these functions drive: School-zone safety tab (schoolTab* fns + escapeXML/KML export).
- In DevTools confirm `typeof window.updateSchoolTabTable` → `'function'` and
  `typeof CL.assets.schoolTab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/assets/school-tab.js
git checkout -- app/index.html
rm app/modules/assets/school-tab.js
```
