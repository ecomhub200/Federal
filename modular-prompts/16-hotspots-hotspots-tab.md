# CC Modular Extraction Prompt 16 — `app/modules/hotspots/hotspots-tab.js`
...
# 1. Locate the block. Snapshot range: L61500-L63344 (feature: Hot Spots tab: clustering, detail panel, filter summaries.)
grep -nE 'function +analyzeHotspots\b|const +analyzeHotspots\b|let +analyzeHotspots\b|window\.analyzeHotspots\b|analyzeHotspots *= *function|analyzeHotspots *= *async|analyzeHotspots *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=61500 && ($2+0)<=63344' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/hotspots/hotspots-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/dashboard/dashboard-tab.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L61500–L63344, ~1845 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `analyzeHotspots` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/hotspots/hotspots-tab.js`:

```js
/**
 * CL hotspots.tab module
 *
 * Extracted from app/index.html (snapshot L61500-L63344) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/16-hotspots-hotspots-tab.md.
 * Responsibility: Hot Spots tab: clustering, detail panel, filter summaries.
 *
 * Public API (back-compat dual exposure):
 *   - window.analyzeHotspots → CL.hotspots.tab.analyzeHotspots
 *
 * Depends on (must load before this file): `analysis/hotspots`, `spatial/aggregate-loader`
 */
(function(){
  'use strict';
  ...
  window.analyzeHotspots = analyzeHotspots; CL.hotspots.analyzeHotspots = analyzeHotspots;
  CL._registerModule('hotspots/hotspots-tab');
})();
```
Registration string is **path-style** `'hotspots/hotspots-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: hotspots/hotspots-tab`). If `CL.hotspots.tab` needs a new
...
`<script src="modules/dashboard/dashboard-tab.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/hotspots/hotspots-tab.js"></script>
```
...
wc -l app/modules/hotspots/hotspots-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +analyzeHotspots\b|const +analyzeHotspots\b|let +analyzeHotspots\b|window\.analyzeHotspots\b|analyzeHotspots *= *function|analyzeHotspots *= *async|analyzeHotspots *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +analyzeHotspots\b|const +analyzeHotspots\b|let +analyzeHotspots\b|window\.analyzeHotspots\b|analyzeHotspots *= *function|analyzeHotspots *= *async|analyzeHotspots *= *\(' app/modules/hotspots/hotspots-tab.js       # expected: the anchors present
grep -c '<script src="modules/hotspots/hotspots-tab.js"></script>' app/index.html  # 1
node --check app/modules/hotspots/hotspots-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: hotspots/hotspots-tab`.
...
```
- Exercise the feature these functions drive: Hot Spots tab: clustering, detail panel, filter summaries.
- In DevTools confirm `typeof window.analyzeHotspots` → `'function'` and
  `typeof CL.hotspots.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/hotspots/hotspots-tab.js
git checkout -- app/index.html
rm app/modules/hotspots/hotspots-tab.js
```
