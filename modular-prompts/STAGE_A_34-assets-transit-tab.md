# Stage A Conversion 34 — `app/modules/assets/transit-tab.js` IIFE → ESM
...
- **Module:** `app/modules/assets/transit-tab.js`
- **CL namespace:** `assets/transit-tab`
- **Baseline size (IIFE era):** 1028 lines (re-snapshot before editing)
...
- [ ] `app/modules/assets/transit-tab.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/assets/transit-tab.js` passes (pre-state).
- [ ] `wc -l app/modules/assets/transit-tab.js` snapshotted (expect ~1028).
...
5. Keep `CL._registerModule('assets/transit-tab');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/assets/transit-tab.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/assets/transit-tab.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: assets/transit-tab`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/assets/transit-tab.js
```
