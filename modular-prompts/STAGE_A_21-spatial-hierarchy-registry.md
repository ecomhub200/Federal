# Stage A Conversion 21 — `app/modules/spatial/hierarchy-registry.js` IIFE → ESM
...
- **Module:** `app/modules/spatial/hierarchy-registry.js`
- **CL namespace:** `spatial/hierarchy-registry`
- **Baseline size (IIFE era):** 130 lines (re-snapshot before editing)
...
- [ ] `app/modules/spatial/hierarchy-registry.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/spatial/hierarchy-registry.js` passes (pre-state).
- [ ] `wc -l app/modules/spatial/hierarchy-registry.js` snapshotted (expect ~130).
...
5. Keep `CL._registerModule('spatial/hierarchy-registry');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/spatial/hierarchy-registry.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/spatial/hierarchy-registry.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: spatial/hierarchy-registry`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/spatial/hierarchy-registry.js
```
