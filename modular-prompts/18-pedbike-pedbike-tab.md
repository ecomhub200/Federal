# CC Modular Extraction Prompt 18 — `app/modules/pedbike/pedbike-tab.js`
...
# 1. Locate the block. Snapshot range: L66772-L68500 (feature: Ped/Bike dual analysis tab + location charts.)
grep -nE 'function +updatePedBikeTab\b|const +updatePedBikeTab\b|let +updatePedBikeTab\b|window\.updatePedBikeTab\b|updatePedBikeTab *= *function|updatePedBikeTab *= *async|updatePedBikeTab *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=66772 && ($2+0)<=68500' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/pedbike/pedbike-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/intersection/intersection-tab.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L66772–L68500, ~1729 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `updatePedBikeTab` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/pedbike/pedbike-tab.js`:

```js
/**
 * CL pedbike.tab module
 *
 * Extracted from app/index.html (snapshot L66772-L68500) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/18-pedbike-pedbike-tab.md.
 * Responsibility: Ped/Bike dual analysis tab + location charts.
 *
 * Public API (back-compat dual exposure):
 *   - window.updatePedBikeTab → CL.pedbike.tab.updatePedBikeTab
 *
 * Depends on (must load before this file): `analysis/crash-profile`
 */
(function(){
  'use strict';
  ...
  window.updatePedBikeTab = updatePedBikeTab; CL.pedbike.updatePedBikeTab = updatePedBikeTab;
  CL._registerModule('pedbike/pedbike-tab');
})();
```
Registration string is **path-style** `'pedbike/pedbike-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: pedbike/pedbike-tab`). If `CL.pedbike.tab` needs a new
...
`<script src="modules/intersection/intersection-tab.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/pedbike/pedbike-tab.js"></script>
```
...
wc -l app/modules/pedbike/pedbike-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +updatePedBikeTab\b|const +updatePedBikeTab\b|let +updatePedBikeTab\b|window\.updatePedBikeTab\b|updatePedBikeTab *= *function|updatePedBikeTab *= *async|updatePedBikeTab *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +updatePedBikeTab\b|const +updatePedBikeTab\b|let +updatePedBikeTab\b|window\.updatePedBikeTab\b|updatePedBikeTab *= *function|updatePedBikeTab *= *async|updatePedBikeTab *= *\(' app/modules/pedbike/pedbike-tab.js       # expected: the anchors present
grep -c '<script src="modules/pedbike/pedbike-tab.js"></script>' app/index.html  # 1
node --check app/modules/pedbike/pedbike-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: pedbike/pedbike-tab`.
...
```
- Exercise the feature these functions drive: Ped/Bike dual analysis tab + location charts.
- In DevTools confirm `typeof window.updatePedBikeTab` → `'function'` and
  `typeof CL.pedbike.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/pedbike/pedbike-tab.js
git checkout -- app/index.html
rm app/modules/pedbike/pedbike-tab.js
```
