# Stage A Conversion 40 — `app/modules/scorecard/scorecard.js` IIFE → ESM
...
- **Module:** `app/modules/scorecard/scorecard.js`
- **CL namespace:** `scorecard/scorecard`
- **Baseline size (IIFE era):** 973 lines (re-snapshot before editing)
...
- [ ] `app/modules/scorecard/scorecard.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/scorecard/scorecard.js` passes (pre-state).
- [ ] `wc -l app/modules/scorecard/scorecard.js` snapshotted (expect ~973).
...
3. `export` every member previously assigned to `CL.scorecard.*` or
   `window.*` (public API). Leave module-private helpers unexported.

### Imports to ADD (top of file)
```js
import { /* prewarm members */ } from '../data/prewarm.js';
```
...
4. Keep transitional `CL.scorecard = CL.scorecard || {}; CL.scorecard.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('scorecard/scorecard');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/scorecard/scorecard.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/scorecard/scorecard.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: scorecard/scorecard`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/scorecard/scorecard.js
```
