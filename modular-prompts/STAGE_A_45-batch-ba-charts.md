# Stage A Conversion 45 — `app/modules/batch-ba/batch-ba-charts.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-charts.js`
- **CL namespace:** `batch-ba/charts`
- **Baseline size (IIFE era):** 493 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-charts.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-charts.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-charts.js` snapshotted (expect ~493).
...
5. Keep `CL._registerModule('batch-ba/charts');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-charts.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-charts.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/charts`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-charts.js
```
