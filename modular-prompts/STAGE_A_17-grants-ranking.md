# Stage A Conversion 17 — `app/modules/grants/ranking.js` IIFE → ESM
...
- **Module:** `app/modules/grants/ranking.js`
- **CL namespace:** `grants/ranking`
- **Baseline size (IIFE era):** 147 lines (re-snapshot before editing)
...
- [ ] `app/modules/grants/ranking.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/grants/ranking.js` passes (pre-state).
- [ ] `wc -l app/modules/grants/ranking.js` snapshotted (expect ~147).
...
3. `export` every member previously assigned to `CL.grants.*` or
...
4. Keep transitional `CL.grants = CL.grants || {}; CL.grants.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('grants/ranking');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/grants/ranking.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/grants/ranking.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: grants/ranking`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/grants/ranking.js
```
