# Stage A Conversion 07 — `app/modules/data/prewarm.js` IIFE → ESM
...
- **Module:** `app/modules/data/prewarm.js`
- **CL namespace:** `data/prewarm`
- **Baseline size (IIFE era):** 307 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/prewarm.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/prewarm.js` passes (pre-state).
- [ ] `wc -l app/modules/data/prewarm.js` snapshotted (expect ~307).
...
5. Keep `CL._registerModule('data/prewarm');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/prewarm.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/prewarm.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/prewarm`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/prewarm.js
```
