# Stage A Conversion 49 — `app/modules/batch-ba/batch-ba-export-pdf.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-export-pdf.js`
- **CL namespace:** `batch-ba/export-pdf`
- **Baseline size (IIFE era):** 500 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-export-pdf.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-export-pdf.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-export-pdf.js` snapshotted (expect ~500).
...
```js
import { /* epdo */ } from '../core/epdo.js';
// + batch-ba sibling imports (see cycle note)
```
...
5. Keep `CL._registerModule('batch-ba/export-pdf');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-export-pdf.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-export-pdf.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/export-pdf`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-export-pdf.js
```
