# CC Modular Extraction Prompt 46 — `app/modules/app/bootstrap.js`
...
# 1. Locate the block. Snapshot range: L33186-L33600 (feature: DOMContentLoaded init chain + autoLoadCrashData gateway.)
grep -nE 'function +autoLoadCrashData\b|const +autoLoadCrashData\b|let +autoLoadCrashData\b|window\.autoLoadCrashData\b|autoLoadCrashData *= *function|autoLoadCrashData *= *async|autoLoadCrashData *= *\(|function +_supabaseTabReady\b|const +_supabaseTabReady\b|let +_supabaseTabReady\b|window\._supabaseTabReady\b|_supabaseTabReady *= *function|_supabaseTabReady *= *async|_supabaseTabReady *= *\(|function +attemptAutoload\b|const +attemptAutoload\b|let +attemptAutoload\b|window\.attemptAutoload\b|attemptAutoload *= *function|attemptAutoload *= *async|attemptAutoload *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=33186 && ($2+0)<=33600' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/app/bootstrap.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/app/tab-dispatcher.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L33186–L33600, ~415 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `autoLoadCrashData` (and the helpers between it and the next named decl)
- `_supabaseTabReady` (and the helpers between it and the next named decl)
- `attemptAutoload` (and the helpers between it and the next named decl)

Globals (module-private — move with the code):
- `crashState`
Event listeners (move with the code, do not duplicate):
- `DOMContentLoaded (remaining boot listeners)`
- `crashDataLoaded`
...
## §2 Where to put it
Create `app/modules/app/bootstrap.js`:

```js
/**
 * CL app.bootstrap module
 *
 * Extracted from app/index.html (snapshot L33186-L33600) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/46-app-bootstrap.md.
 * Responsibility: DOMContentLoaded init chain + autoLoadCrashData gateway.
 *
 * Public API (back-compat dual exposure):
 *   - window.autoLoadCrashData → CL.app.bootstrap.autoLoadCrashData
 *   - window._supabaseTabReady → CL.app.bootstrap._supabaseTabReady
 *   - window.attemptAutoload → CL.app.bootstrap.attemptAutoload
 *
 * Depends on (must load before this file): `app/tab-dispatcher`, `spatial/r2-resolve`, `data/supabase-bridge`
 */
(function(){
  'use strict';
  ...
  window.CL = window.CL || {};
  CL.app = CL.app || {};
  window.autoLoadCrashData = autoLoadCrashData; CL.app.autoLoadCrashData = autoLoadCrashData;
  window._supabaseTabReady = _supabaseTabReady; CL.app._supabaseTabReady = _supabaseTabReady;
  window.attemptAutoload = attemptAutoload; CL.app.attemptAutoload = attemptAutoload;
  CL._registerModule('app/bootstrap');
})();
```
Registration string is **path-style** `'app/bootstrap'` (matches `loader.js`
convention — logs `[CL] Module loaded: app/bootstrap`). If `CL.app.bootstrap` needs a new
...
`<script src="modules/app/tab-dispatcher.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/app/bootstrap.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +autoLoadCrashData\b|const +autoLoadCrashData\b|let +autoLoadCrashData\b|window\.autoLoadCrashData\b|autoLoadCrashData *= *function|autoLoadCrashData *= *async|autoLoadCrashData *= *\(|function +_supabaseTabReady\b|const +_supabaseTabReady\b|let +_supabaseTabReady\b|window\._supabaseTabReady\b|_supabaseTabReady *= *function|_supabaseTabReady *= *async|_supabaseTabReady *= *\(|function +attemptAutoload\b|const +attemptAutoload\b|let +attemptAutoload\b|window\.attemptAutoload\b|attemptAutoload *= *function|attemptAutoload *= *async|attemptAutoload *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +autoLoadCrashData\b|const +autoLoadCrashData\b|let +autoLoadCrashData\b|window\.autoLoadCrashData\b|autoLoadCrashData *= *function|autoLoadCrashData *= *async|autoLoadCrashData *= *\(|function +_supabaseTabReady\b|const +_supabaseTabReady\b|let +_supabaseTabReady\b|window\._supabaseTabReady\b|_supabaseTabReady *= *function|_supabaseTabReady *= *async|_supabaseTabReady *= *\(|function +attemptAutoload\b|const +attemptAutoload\b|let +attemptAutoload\b|window\.attemptAutoload\b|attemptAutoload *= *function|attemptAutoload *= *async|attemptAutoload *= *\(' app/modules/app/bootstrap.js       # expected: the anchors present
grep -c '<script src="modules/app/bootstrap.js"></script>' app/index.html  # 1
node --check app/modules/app/bootstrap.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: app/bootstrap`.
...
```
- Exercise the feature these functions drive: DOMContentLoaded init chain + autoLoadCrashData gateway.
- In DevTools confirm `typeof window.autoLoadCrashData` → `'function'` and
  `typeof CL.app.bootstrap` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/app/bootstrap.js
git checkout -- app/index.html
rm app/modules/app/bootstrap.js
```
