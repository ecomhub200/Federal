# Stage A Conversion 10 — `app/modules/data/lazy-loader.js` IIFE → ESM
...
- **Module:** `app/modules/data/lazy-loader.js`
- **CL namespace:** `data/lazyLoader`
- **Baseline size (IIFE era):** 227 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/lazy-loader.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/lazy-loader.js` passes (pre-state).
- [ ] `wc -l app/modules/data/lazy-loader.js` snapshotted (expect ~227).
...
5. Keep `CL._registerModule('data/lazyLoader');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/lazy-loader.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/lazy-loader.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/lazyLoader`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/lazy-loader.js
```
