# Stage A Conversion 03 — `app/modules/core/epdo.js` IIFE → ESM
...
- **Module:** `app/modules/core/epdo.js`
- **CL namespace:** `core/epdo`
- **Baseline size (IIFE era):** 32 lines (re-snapshot before editing)
...
- [ ] `app/modules/core/epdo.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/core/epdo.js` passes (pre-state).
- [ ] `wc -l app/modules/core/epdo.js` snapshotted (expect ~32).
...
5. Keep `CL._registerModule('core/epdo');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/core/epdo.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/core/epdo.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: core/epdo`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/core/epdo.js
```
