# CC Modular Extraction Prompt 31 — `app/modules/cmf/cmf-search.js`
...
# 1. Locate the block. Snapshot range: L44528-L48199 (feature: CMF database load + location population + search.)
grep -nE 'function +loadCMFDatabase\b|const +loadCMFDatabase\b|let +loadCMFDatabase\b|window\.loadCMFDatabase\b|loadCMFDatabase *= *function|loadCMFDatabase *= *async|loadCMFDatabase *= *\(|function +populateCMFLocations\b|const +populateCMFLocations\b|let +populateCMFLocations\b|window\.populateCMFLocations\b|populateCMFLocations *= *function|populateCMFLocations *= *async|populateCMFLocations *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=44528 && ($2+0)<=48199' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/cmf/cmf-search.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/grants/grants-ui.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L44528–L48199, ~3672 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `loadCMFDatabase` (and the helpers between it and the next named decl)
- `populateCMFLocations` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/cmf/cmf-search.js`:

```js
/**
 * CL cmf.search module
 *
 * Extracted from app/index.html (snapshot L44528-L48199) on 2026-05-15.
 ...
 *   - window.loadCMFDatabase → CL.cmf.search.loadCMFDatabase
 *   - window.populateCMFLocations → CL.cmf.search.populateCMFLocations
 *
...
```
Registration string is **path-style** `'cmf/cmf-search'` (matches `loader.js`
convention — logs `[CL] Module loaded: cmf/cmf-search`). If `CL.cmf.search` needs a new
...
`<script src="modules/grants/grants-ui.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/cmf/cmf-search.js"></script>
```
...
wc -l app/modules/cmf/cmf-search.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +loadCMFDatabase\b|const +loadCMFDatabase\b|let +loadCMFDatabase\b|window\.loadCMFDatabase\b|loadCMFDatabase *= *function|loadCMFDatabase *= *async|loadCMFDatabase *= *\(|function +populateCMFLocations\b|const +populateCMFLocations\b|let +populateCMFLocations\b|window\.populateCMFLocations\b|populateCMFLocations *= *function|populateCMFLocations *= *async|populateCMFLocations *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +loadCMFDatabase\b|const +loadCMFDatabase\b|let +loadCMFDatabase\b|window\.loadCMFDatabase\b|loadCMFDatabase *= *function|loadCMFDatabase *= *async|loadCMFDatabase *= *\(|function +populateCMFLocations\b|const +populateCMFLocations\b|let +populateCMFLocations\b|window\.populateCMFLocations\b|populateCMFLocations *= *function|populateCMFLocations *= *async|populateCMFLocations *= *\(' app/modules/cmf/cmf-search.js       # expected: the anchors present
grep -c '<script src="modules/cmf/cmf-search.js"></script>' app/index.html  # 1
node --check app/modules/cmf/cmf-search.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: cmf/cmf-search`.
...
```
- Exercise the feature these functions drive: CMF database load + location population + search.
- In DevTools confirm `typeof window.loadCMFDatabase` → `'function'` and
  `typeof CL.cmf.search` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/cmf/cmf-search.js
git checkout -- app/index.html
rm app/modules/cmf/cmf-search.js
```
