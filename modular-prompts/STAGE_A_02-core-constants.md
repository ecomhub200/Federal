# Stage A Conversion 02 — `app/modules/core/constants.js` IIFE → ESM
...
- **Module:** `app/modules/core/constants.js`
- **CL namespace:** `core/constants`
- **Baseline size (IIFE era):** 149 lines (re-snapshot before editing)
...
- [ ] `app/modules/core/constants.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/core/constants.js` passes (pre-state).
- [ ] `wc -l app/modules/core/constants.js` snapshotted (expect ~149).
...
5. Keep `CL._registerModule('core/constants');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/core/constants.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/core/constants.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: core/constants`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/core/constants.js
```
