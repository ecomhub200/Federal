# Stage A Conversion 33 — `app/modules/assets/school-tab.js` IIFE → ESM
...
- **Module:** `app/modules/assets/school-tab.js`
- **CL namespace:** `assets/school-tab`
- **Baseline size (IIFE era):** 509 lines (re-snapshot before editing)
...
- [ ] `app/modules/assets/school-tab.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/assets/school-tab.js` passes (pre-state).
- [ ] `wc -l app/modules/assets/school-tab.js` snapshotted (expect ~509).
...
5. Keep `CL._registerModule('assets/school-tab');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/assets/school-tab.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/assets/school-tab.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: assets/school-tab`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/assets/school-tab.js
```
