# CC Modular Extraction Prompt 41 — `app/modules/ai/ai-domain-knowledge.js`
...
# 1. Locate the block. Snapshot range: L86463-L88000 (feature: Domain Knowledge tab init + retrieval.)
...
awk -F'|' 'NR>9 && ($2+0)>=86463 && ($2+0)<=88000' INDEX_MAP_part*.md
...
BLK_END]** confirmed in §0 (snapshot L86463–L88000, ~1538 lines). The exact
declarations are the `INDEX_MAP_part3.md` rows inside that range. Anchor
declarations (use to find the block):
- `(domain knowledge fns)` (and the helpers between it and the next named decl)

Event listeners (move with the code, do not duplicate):
- `DOMContentLoaded@86463`
...
## §2 Where to put it
Create `app/modules/ai/ai-domain-knowledge.js`:

```js
/**
 * CL ai.domainKnowledge module
 *
 * Extracted from app/index.html (snapshot L86463-L88000) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/41-ai-ai-domain-knowledge.md.
...
(function(){
  ...
  CL._registerModule('ai/ai-domain-knowledge');
})();
```
Registration string is **path-style** `'ai/ai-domain-knowledge'` (matches `loader.js`
convention — logs `[CL] Module loaded: ai/ai-domain-knowledge`). If `CL.ai.domainKnowledge` needs a new
...
```html
<script src="modules/ai/ai-domain-knowledge.js"></script>
```
...
wc -l app/modules/ai/ai-domain-knowledge.js                      # ≈ extracted + ~25 wrapper
grep -nE 'ai[A-Z]|[Aa]ssistant' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'ai[A-Z]|[Aa]ssistant' app/modules/ai/ai-domain-knowledge.js       # expected: the anchors present
grep -c '<script src="modules/ai/ai-domain-knowledge.js"></script>' app/index.html  # 1
node --check app/modules/ai/ai-domain-knowledge.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: ai/ai-domain-knowledge`.
...
```
- Exercise the feature these functions drive: Domain Knowledge tab init + retrieval.
- In DevTools confirm `typeof window.domain` → `'function'` and
  `typeof CL.ai.domainKnowledge` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/ai/ai-domain-knowledge.js
git checkout -- app/index.html
rm app/modules/ai/ai-domain-knowledge.js
```
