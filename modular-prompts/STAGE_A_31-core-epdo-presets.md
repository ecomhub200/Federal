# Stage A Conversion 31 — `app/modules/core/epdo-presets.js` IIFE → ESM
...
- **Module:** `app/modules/core/epdo-presets.js`
- **CL namespace:** `core/epdo-presets`
- **Baseline size (IIFE era):** 267 lines (re-snapshot before editing)
...
- [ ] `app/modules/core/epdo-presets.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/core/epdo-presets.js` passes (pre-state).
- [ ] `wc -l app/modules/core/epdo-presets.js` snapshotted (expect ~267).
...
3. `export` every member previously assigned to `CL.core.*` or
...
4. Keep transitional `CL.core = CL.core || {}; CL.core.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('core/epdo-presets');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/core/epdo-presets.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/core/epdo-presets.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: core/epdo-presets`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/core/epdo-presets.js
```
