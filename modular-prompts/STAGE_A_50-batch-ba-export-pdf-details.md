# Stage A Conversion 50 — `app/modules/batch-ba/batch-ba-export-pdf-details.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-export-pdf-details.js`
- **CL namespace:** `batch-ba/export-pdf-details`
- **Baseline size (IIFE era):** 352 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-export-pdf-details.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-export-pdf-details.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-export-pdf-details.js` snapshotted (expect ~352).
...
5. Keep `CL._registerModule('batch-ba/export-pdf-details');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-export-pdf-details.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-export-pdf-details.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/export-pdf-details`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-export-pdf-details.js
```
