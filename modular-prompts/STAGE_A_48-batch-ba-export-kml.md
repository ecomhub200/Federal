# Stage A Conversion 48 — `app/modules/batch-ba/batch-ba-export-kml.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-export-kml.js`
- **CL namespace:** `batch-ba/export-kml`
- **Baseline size (IIFE era):** 160 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-export-kml.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-export-kml.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-export-kml.js` snapshotted (expect ~160).
...
5. Keep `CL._registerModule('batch-ba/export-kml');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-export-kml.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-export-kml.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/export-kml`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-export-kml.js
```
