# CC Modular Extraction Prompt 21 — `app/modules/fatal-speeding/fatal-speeding-tab.js`
...
# 1. Locate the block. Snapshot range: L109100-L113700 (feature: Fatal & Speeding matrix + severity/factor filtering.)
grep -nE 'function +initFatalSpeedingTab\b|const +initFatalSpeedingTab\b|let +initFatalSpeedingTab\b|window\.initFatalSpeedingTab\b|initFatalSpeedingTab *= *function|initFatalSpeedingTab *= *async|initFatalSpeedingTab *= *\(|function +applyFSFilters\b|const +applyFSFilters\b|let +applyFSFilters\b|window\.applyFSFilters\b|applyFSFilters *= *function|applyFSFilters *= *async|applyFSFilters *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=109100 && ($2+0)<=113700' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/fatal-speeding/fatal-speeding-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/crash-tree/crash-tree-tab.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L109100–L113700, ~4601 lines). The exact
declarations are the `INDEX_MAP_part3.md` rows inside that range. Anchor
declarations (use to find the block):
- `initFatalSpeedingTab` (and the helpers between it and the next named decl)
- `applyFSFilters` (and the helpers between it and the next named decl)
...
```js
/**
 * CL fatalSpeeding.tab module
 *
 * Extracted from app/index.html (snapshot L109100-L113700) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/21-fatal-speeding-fatal-speeding-tab.md.
...
(function(){
  ...
  window.CL = window.CL || {};
  CL.fatal-speeding = CL.fatal-speeding || {};
  window.initFatalSpeedingTab = initFatalSpeedingTab; CL.fatal-speeding.initFatalSpeedingTab = initFatalSpeedingTab;
  window.applyFSFilters = applyFSFilters; CL.fatal-speeding.applyFSFilters = applyFSFilters;
  CL._registerModule('fatal-speeding/fatal-speeding-tab');
})();
```
Registration string is **path-style** `'fatal-speeding/fatal-speeding-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: fatal-speeding/fatal-speeding-tab`). If `CL.fatalSpeeding.tab` needs a new
...
`<script src="modules/crash-tree/crash-tree-tab.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/fatal-speeding/fatal-speeding-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initFatalSpeedingTab\b|const +initFatalSpeedingTab\b|let +initFatalSpeedingTab\b|window\.initFatalSpeedingTab\b|initFatalSpeedingTab *= *function|initFatalSpeedingTab *= *async|initFatalSpeedingTab *= *\(|function +applyFSFilters\b|const +applyFSFilters\b|let +applyFSFilters\b|window\.applyFSFilters\b|applyFSFilters *= *function|applyFSFilters *= *async|applyFSFilters *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initFatalSpeedingTab\b|const +initFatalSpeedingTab\b|let +initFatalSpeedingTab\b|window\.initFatalSpeedingTab\b|initFatalSpeedingTab *= *function|initFatalSpeedingTab *= *async|initFatalSpeedingTab *= *\(|function +applyFSFilters\b|const +applyFSFilters\b|let +applyFSFilters\b|window\.applyFSFilters\b|applyFSFilters *= *function|applyFSFilters *= *async|applyFSFilters *= *\(' app/modules/fatal-speeding/fatal-speeding-tab.js       # expected: the anchors present
grep -c '<script src="modules/fatal-speeding/fatal-speeding-tab.js"></script>' app/index.html  # 1
node --check app/modules/fatal-speeding/fatal-speeding-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: fatal-speeding/fatal-speeding-tab`.
...
```
- Exercise the feature these functions drive: Fatal & Speeding matrix + severity/factor filtering.
- In DevTools confirm `typeof window.initFatalSpeedingTab` → `'function'` and
  `typeof CL.fatalSpeeding.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/fatal-speeding/fatal-speeding-tab.js
git checkout -- app/index.html
rm app/modules/fatal-speeding/fatal-speeding-tab.js
```
