# CC Modular Extraction Prompt 23 — `app/modules/core/epdo-presets.js`
...
test -f app/modules/core/epdo-presets.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/warrants/signal-thresholds.js"></script>' app/index.html   # expected: 1 match
```
...
- `saveCustomEPDOWeights` (and the helpers between it and the next named decl)
- `recalculateAllEPDO` (and the helpers between it and the next named decl)
- `applyStateDefaultEPDO` (and the helpers between it and the next named decl)
- `updateEPDOPresetUI` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/core/epdo-presets.js`:
 ...
 *   - window.loadEPDOPreset → CL.core.epdoPresets.loadEPDOPreset
 *   - window.saveCustomEPDOWeights → CL.core.epdoPresets.saveCustomEPDOWeights
 *   - window.recalculateAllEPDO → CL.core.epdoPresets.recalculateAllEPDO
 *   - window.applyStateDefaultEPDO → CL.core.epdoPresets.applyStateDefaultEPDO
 *   - window.updateEPDOPresetUI → CL.core.epdoPresets.updateEPDOPresetUI
 *
...
```
Registration string is **path-style** `'core/epdo-presets'` (matches `loader.js`
convention — logs `[CL] Module loaded: core/epdo-presets`). If `CL.core.epdoPresets` needs a new
...
`<script src="modules/warrants/signal-thresholds.js"></script>` (this places it in the **EARLY** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/core/epdo-presets.js"></script>
```
...
wc -l app/modules/core/epdo-presets.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +loadEPDOPreset\b|const +loadEPDOPreset\b|let +loadEPDOPreset\b|window\.loadEPDOPreset\b|loadEPDOPreset *= *function|loadEPDOPreset *= *async|loadEPDOPreset *= *\(|function +saveCustomEPDOWeights\b|const +saveCustomEPDOWeights\b|let +saveCustomEPDOWeights\b|window\.saveCustomEPDOWeights\b|saveCustomEPDOWeights *= *function|saveCustomEPDOWeights *= *async|saveCustomEPDOWeights *= *\(|function +recalculateAllEPDO\b|const +recalculateAllEPDO\b|let +recalculateAllEPDO\b|window\.recalculateAllEPDO\b|recalculateAllEPDO *= *function|recalculateAllEPDO *= *async|recalculateAllEPDO *= *\(|function +applyStateDefaultEPDO\b|const +applyStateDefaultEPDO\b|let +applyStateDefaultEPDO\b|window\.applyStateDefaultEPDO\b|applyStateDefaultEPDO *= *function|applyStateDefaultEPDO *= *async|applyStateDefaultEPDO *= *\(|function +updateEPDOPresetUI\b|const +updateEPDOPresetUI\b|let +updateEPDOPresetUI\b|window\.updateEPDOPresetUI\b|updateEPDOPresetUI *= *function|updateEPDOPresetUI *= *async|updateEPDOPresetUI *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +loadEPDOPreset\b|const +loadEPDOPreset\b|let +loadEPDOPreset\b|window\.loadEPDOPreset\b|loadEPDOPreset *= *function|loadEPDOPreset *= *async|loadEPDOPreset *= *\(|function +saveCustomEPDOWeights\b|const +saveCustomEPDOWeights\b|let +saveCustomEPDOWeights\b|window\.saveCustomEPDOWeights\b|saveCustomEPDOWeights *= *function|saveCustomEPDOWeights *= *async|saveCustomEPDOWeights *= *\(|function +recalculateAllEPDO\b|const +recalculateAllEPDO\b|let +recalculateAllEPDO\b|window\.recalculateAllEPDO\b|recalculateAllEPDO *= *function|recalculateAllEPDO *= *async|recalculateAllEPDO *= *\(|function +applyStateDefaultEPDO\b|const +applyStateDefaultEPDO\b|let +applyStateDefaultEPDO\b|window\.applyStateDefaultEPDO\b|applyStateDefaultEPDO *= *function|applyStateDefaultEPDO *= *async|applyStateDefaultEPDO *= *\(|function +updateEPDOPresetUI\b|const +updateEPDOPresetUI\b|let +updateEPDOPresetUI\b|window\.updateEPDOPresetUI\b|updateEPDOPresetUI *= *function|updateEPDOPresetUI *= *async|updateEPDOPresetUI *= *\(' app/modules/core/epdo-presets.js       # expected: the anchors present
grep -c '<script src="modules/core/epdo-presets.js"></script>' app/index.html  # 1
node --check app/modules/core/epdo-presets.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: core/epdo-presets`.
...
```
- Exercise the feature these functions drive: EPDO preset load/save/UI + per-state weights.
- In DevTools confirm `typeof window.loadEPDOPreset` → `'function'` and
  `typeof CL.core.epdoPresets` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/core/epdo-presets.js
git checkout -- app/index.html
rm app/modules/core/epdo-presets.js
```
