# Stage A Conversion 26 — `app/modules/spatial/r2-resolve.js` IIFE → ESM
...
- **Module:** `app/modules/spatial/r2-resolve.js`
- **CL namespace:** `spatial/r2-resolve`
- **Baseline size (IIFE era):** 541 lines (re-snapshot before editing)
...
- [ ] `app/modules/spatial/r2-resolve.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/spatial/r2-resolve.js` passes (pre-state).
- [ ] `wc -l app/modules/spatial/r2-resolve.js` snapshotted (expect ~541).
...
5. Keep `CL._registerModule('spatial/r2-resolve');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/spatial/r2-resolve.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/spatial/r2-resolve.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: spatial/r2-resolve`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/spatial/r2-resolve.js
```
