# CC Modular Extraction Prompt 01 — `app/modules/spatial/hierarchy-registry.js`
...
# 1. Locate the block. Snapshot range: L22115-L22219 (feature: Jurisdiction hierarchy registry (regions/TPRs/MPOs/counties lookups).)
grep -nE 'function +HierarchyRegistry\b|const +HierarchyRegistry\b|let +HierarchyRegistry\b|window\.HierarchyRegistry\b|HierarchyRegistry *= *function|HierarchyRegistry *= *async|HierarchyRegistry *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=22115 && ($2+0)<=22219' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/spatial/hierarchy-registry.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/grants/ranking.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L22115–L22219, ~105 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `HierarchyRegistry` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/spatial/hierarchy-registry.js`:
 ...
 *   - window.HierarchyRegistry → CL.spatial.hierarchyRegistry.HierarchyRegistry
 *
...
```
Registration string is **path-style** `'spatial/hierarchy-registry'` (matches `loader.js`
convention — logs `[CL] Module loaded: spatial/hierarchy-registry`). If `CL.spatial.hierarchyRegistry` needs a new
...
`<script src="modules/grants/ranking.js"></script>` (this places it in the **EARLY** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/spatial/hierarchy-registry.js"></script>
```
...
wc -l app/modules/spatial/hierarchy-registry.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +HierarchyRegistry\b|const +HierarchyRegistry\b|let +HierarchyRegistry\b|window\.HierarchyRegistry\b|HierarchyRegistry *= *function|HierarchyRegistry *= *async|HierarchyRegistry *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +HierarchyRegistry\b|const +HierarchyRegistry\b|let +HierarchyRegistry\b|window\.HierarchyRegistry\b|HierarchyRegistry *= *function|HierarchyRegistry *= *async|HierarchyRegistry *= *\(' app/modules/spatial/hierarchy-registry.js       # expected: the anchors present
grep -c '<script src="modules/spatial/hierarchy-registry.js"></script>' app/index.html  # 1
node --check app/modules/spatial/hierarchy-registry.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: spatial/hierarchy-registry`.
...
```
- Exercise the feature these functions drive: Jurisdiction hierarchy registry (regions/TPRs/MPOs/counties lookups).
- In DevTools confirm `typeof window.HierarchyRegistry` → `'function'` and
  `typeof CL.spatial.hierarchyRegistry` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/spatial/hierarchy-registry.js
git checkout -- app/index.html
rm app/modules/spatial/hierarchy-registry.js
```
