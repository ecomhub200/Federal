# Stage A Conversion 30 — `app/modules/upload/upload-tier-ui.js` IIFE → ESM
...
- **Module:** `app/modules/upload/upload-tier-ui.js`
- **CL namespace:** `upload/upload-tier-ui`
- **Baseline size (IIFE era):** 599 lines (re-snapshot before editing)
...
- [ ] `app/modules/upload/upload-tier-ui.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/upload/upload-tier-ui.js` passes (pre-state).
- [ ] `wc -l app/modules/upload/upload-tier-ui.js` snapshotted (expect ~599).
...
5. Keep `CL._registerModule('upload/upload-tier-ui');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/upload/upload-tier-ui.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/upload/upload-tier-ui.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: upload/upload-tier-ui`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/upload/upload-tier-ui.js
```
