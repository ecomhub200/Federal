# Stage A Conversion 29 — `app/modules/upload/road-defaults.js` IIFE → ESM
...
- **Module:** `app/modules/upload/road-defaults.js`
- **CL namespace:** `upload/road-defaults`
- **Baseline size (IIFE era):** 266 lines (re-snapshot before editing)
...
- [ ] `app/modules/upload/road-defaults.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/upload/road-defaults.js` passes (pre-state).
- [ ] `wc -l app/modules/upload/road-defaults.js` snapshotted (expect ~266).
...
5. Keep `CL._registerModule('upload/road-defaults');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/upload/road-defaults.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/upload/road-defaults.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: upload/road-defaults`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/upload/road-defaults.js
```
