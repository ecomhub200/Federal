# CC Modular Extraction Prompt 15 — `app/modules/dashboard/dashboard-tab.js`
...
test -f app/modules/dashboard/dashboard-tab.js && echo "ABORT: target exists" || echo "OK: target free"
...
## §2 Where to put it
Create `app/modules/dashboard/dashboard-tab.js`:
 ...
 *   - window.updateDashboard → CL.dashboard.tab.updateDashboard
 *
...
(function(){
  ...
  window.updateDashboard = updateDashboard; CL.dashboard.updateDashboard = updateDashboard;
  CL._registerModule('dashboard/dashboard-tab');
})();
```
Registration string is **path-style** `'dashboard/dashboard-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: dashboard/dashboard-tab`). If `CL.dashboard.tab` needs a new
...
```html
<script src="modules/dashboard/dashboard-tab.js"></script>
```
...
wc -l app/modules/dashboard/dashboard-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +updateDashboard\b|const +updateDashboard\b|let +updateDashboard\b|window\.updateDashboard\b|updateDashboard *= *function|updateDashboard *= *async|updateDashboard *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +updateDashboard\b|const +updateDashboard\b|let +updateDashboard\b|window\.updateDashboard\b|updateDashboard *= *function|updateDashboard *= *async|updateDashboard *= *\(' app/modules/dashboard/dashboard-tab.js       # expected: the anchors present
grep -c '<script src="modules/dashboard/dashboard-tab.js"></script>' app/index.html  # 1
node --check app/modules/dashboard/dashboard-tab.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: dashboard/dashboard-tab`.
...
```
- Exercise the feature these functions drive: Dashboard KPI painting + district stats + matview fallback.
- In DevTools confirm `typeof window.updateDashboard` → `'function'` and
  `typeof CL.dashboard.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/dashboard/dashboard-tab.js
git checkout -- app/index.html
rm app/modules/dashboard/dashboard-tab.js
```
