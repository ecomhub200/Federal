# CC Modular Extraction Prompt 43 — `app/modules/reports/reports-custom.js`
...
# 1. Locate the block. Snapshot range: L14001-L16556 (feature: Custom report builder UI.)
...
awk -F'|' 'NR>9 && ($2+0)>=14001 && ($2+0)<=16556' INDEX_MAP_part*.md
...
BLK_END]** confirmed in §0 (snapshot L14001–L16556, ~2556 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `(custom report builder fns)` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/reports/reports-custom.js`:

```js
/**
 * CL reports.custom module
 *
 * Extracted from app/index.html (snapshot L14001-L16556) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/43-reports-reports-custom.md.
...
(function(){
  ...
  CL._registerModule('reports/reports-custom');
})();
```
Registration string is **path-style** `'reports/reports-custom'` (matches `loader.js`
convention — logs `[CL] Module loaded: reports/reports-custom`). If `CL.reports.custom` needs a new
...
```html
<script src="modules/reports/reports-custom.js"></script>
```
...
wc -l app/modules/reports/reports-custom.js                      # ≈ extracted + ~25 wrapper
grep -nE '[Rr]eport' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE '[Rr]eport' app/modules/reports/reports-custom.js       # expected: the anchors present
grep -c '<script src="modules/reports/reports-custom.js"></script>' app/index.html  # 1
node --check app/modules/reports/reports-custom.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: reports/reports-custom`.
...
```
- Exercise the feature these functions drive: Custom report builder UI.
- In DevTools confirm `typeof window.custom` → `'function'` and
  `typeof CL.reports.custom` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/reports/reports-custom.js
git checkout -- app/index.html
rm app/modules/reports/reports-custom.js
```
