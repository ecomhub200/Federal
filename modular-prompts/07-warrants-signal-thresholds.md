# CC Modular Extraction Prompt 07 — `app/modules/warrants/signal-thresholds.js`
...
# 1. Locate the block. Snapshot range: L30362-L30460 (feature: MUTCD signal warrant threshold tables/curves (constants).)
grep -nE 'function +SIGNAL_WARRANT1_THRESHOLDS\b|const +SIGNAL_WARRANT1_THRESHOLDS\b|let +SIGNAL_WARRANT1_THRESHOLDS\b|window\.SIGNAL_WARRANT1_THRESHOLDS\b|SIGNAL_WARRANT1_THRESHOLDS *= *function|SIGNAL_WARRANT1_THRESHOLDS *= *async|SIGNAL_WARRANT1_THRESHOLDS *= *\(|function +SIGNAL_WARRANT2_CURVES\b|const +SIGNAL_WARRANT2_CURVES\b|let +SIGNAL_WARRANT2_CURVES\b|window\.SIGNAL_WARRANT2_CURVES\b|SIGNAL_WARRANT2_CURVES *= *function|SIGNAL_WARRANT2_CURVES *= *async|SIGNAL_WARRANT2_CURVES *= *\(|function +SIGNAL_WARRANT3_CURVES\b|const +SIGNAL_WARRANT3_CURVES\b|let +SIGNAL_WARRANT3_CURVES\b|window\.SIGNAL_WARRANT3_CURVES\b|SIGNAL_WARRANT3_CURVES *= *function|SIGNAL_WARRANT3_CURVES *= *async|SIGNAL_WARRANT3_CURVES *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=30362 && ($2+0)<=30460' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/warrants/signal-thresholds.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/warrants/signal-tmc.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L30362–L30460, ~99 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `SIGNAL_WARRANT1_THRESHOLDS` (and the helpers between it and the next named decl)
- `SIGNAL_WARRANT2_CURVES` (and the helpers between it and the next named decl)
- `SIGNAL_WARRANT3_CURVES` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/warrants/signal-thresholds.js`:
 ...
 *   - window.SIGNAL_WARRANT1_THRESHOLDS → CL.warrants.signalThresholds.SIGNAL_WARRANT1_THRESHOLDS
 *   - window.SIGNAL_WARRANT2_CURVES → CL.warrants.signalThresholds.SIGNAL_WARRANT2_CURVES
 *   - window.SIGNAL_WARRANT3_CURVES → CL.warrants.signalThresholds.SIGNAL_WARRANT3_CURVES
 *
...
```
Registration string is **path-style** `'warrants/signal-thresholds'` (matches `loader.js`
convention — logs `[CL] Module loaded: warrants/signal-thresholds`). If `CL.warrants.signalThresholds` needs a new
...
`<script src="modules/warrants/signal-tmc.js"></script>` (this places it in the **EARLY** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/warrants/signal-thresholds.js"></script>
```
...
wc -l app/modules/warrants/signal-thresholds.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +SIGNAL_WARRANT1_THRESHOLDS\b|const +SIGNAL_WARRANT1_THRESHOLDS\b|let +SIGNAL_WARRANT1_THRESHOLDS\b|window\.SIGNAL_WARRANT1_THRESHOLDS\b|SIGNAL_WARRANT1_THRESHOLDS *= *function|SIGNAL_WARRANT1_THRESHOLDS *= *async|SIGNAL_WARRANT1_THRESHOLDS *= *\(|function +SIGNAL_WARRANT2_CURVES\b|const +SIGNAL_WARRANT2_CURVES\b|let +SIGNAL_WARRANT2_CURVES\b|window\.SIGNAL_WARRANT2_CURVES\b|SIGNAL_WARRANT2_CURVES *= *function|SIGNAL_WARRANT2_CURVES *= *async|SIGNAL_WARRANT2_CURVES *= *\(|function +SIGNAL_WARRANT3_CURVES\b|const +SIGNAL_WARRANT3_CURVES\b|let +SIGNAL_WARRANT3_CURVES\b|window\.SIGNAL_WARRANT3_CURVES\b|SIGNAL_WARRANT3_CURVES *= *function|SIGNAL_WARRANT3_CURVES *= *async|SIGNAL_WARRANT3_CURVES *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +SIGNAL_WARRANT1_THRESHOLDS\b|const +SIGNAL_WARRANT1_THRESHOLDS\b|let +SIGNAL_WARRANT1_THRESHOLDS\b|window\.SIGNAL_WARRANT1_THRESHOLDS\b|SIGNAL_WARRANT1_THRESHOLDS *= *function|SIGNAL_WARRANT1_THRESHOLDS *= *async|SIGNAL_WARRANT1_THRESHOLDS *= *\(|function +SIGNAL_WARRANT2_CURVES\b|const +SIGNAL_WARRANT2_CURVES\b|let +SIGNAL_WARRANT2_CURVES\b|window\.SIGNAL_WARRANT2_CURVES\b|SIGNAL_WARRANT2_CURVES *= *function|SIGNAL_WARRANT2_CURVES *= *async|SIGNAL_WARRANT2_CURVES *= *\(|function +SIGNAL_WARRANT3_CURVES\b|const +SIGNAL_WARRANT3_CURVES\b|let +SIGNAL_WARRANT3_CURVES\b|window\.SIGNAL_WARRANT3_CURVES\b|SIGNAL_WARRANT3_CURVES *= *function|SIGNAL_WARRANT3_CURVES *= *async|SIGNAL_WARRANT3_CURVES *= *\(' app/modules/warrants/signal-thresholds.js       # expected: the anchors present
grep -c '<script src="modules/warrants/signal-thresholds.js"></script>' app/index.html  # 1
node --check app/modules/warrants/signal-thresholds.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: warrants/signal-thresholds`.
...
```
- Exercise the feature these functions drive: MUTCD signal warrant threshold tables/curves (constants).
- In DevTools confirm `typeof window.SIGNAL_WARRANT1_THRESHOLDS` → `'function'` and
  `typeof CL.warrants.signalThresholds` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/warrants/signal-thresholds.js
git checkout -- app/index.html
rm app/modules/warrants/signal-thresholds.js
```
