# Stage A Conversion 13 — `app/modules/ai/context.js` IIFE → ESM
...
- **Module:** `app/modules/ai/context.js`
- **CL namespace:** `ai/context`
- **Baseline size (IIFE era):** 39 lines (re-snapshot before editing)
...
- [ ] `app/modules/ai/context.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/ai/context.js` passes (pre-state).
- [ ] `wc -l app/modules/ai/context.js` snapshotted (expect ~39).
...
3. `export` every member previously assigned to `CL.ai.*` or
...
4. Keep transitional `CL.ai = CL.ai || {}; CL.ai.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('ai/context');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/ai/context.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/ai/context.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: ai/context`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/ai/context.js
```
