# CC Modular Extraction Prompt 27 — `app/modules/grants/grants-rank.js`
...
# 1. Locate the block. Snapshot range: L34986-L37500 (feature: Grant scoring/ranking engine + scoring profiles.)
grep -nE 'function +initGrantModule\b|const +initGrantModule\b|let +initGrantModule\b|window\.initGrantModule\b|initGrantModule *= *function|initGrantModule *= *async|initGrantModule *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=34986 && ($2+0)<=37500' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/grants/grants-rank.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/scorecard/scorecard-choropleth.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L34986–L37500, ~2515 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `initGrantModule` (and the helpers between it and the next named decl)

Globals (module-private — move with the code):
- `grantState`
- `GRANT_SCORING_PROFILES`
...
## §2 Where to put it
Create `app/modules/grants/grants-rank.js`:

```js
/**
 * CL grants.rank module
 *
 * Extracted from app/index.html (snapshot L34986-L37500) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/27-grants-grants-rank.md.
 * Responsibility: Grant scoring/ranking engine + scoring profiles.
 *
 * Public API (back-compat dual exposure):
 *   - window.initGrantModule → CL.grants.rank.initGrantModule
 *
 * Depends on (must load before this file): `grants/ranking`, `core/epdo-presets`
 */
(function(){
  'use strict';
  ...
  window.CL = window.CL || {};
  CL.grants = CL.grants || {};
  window.initGrantModule = initGrantModule; CL.grants.initGrantModule = initGrantModule;
  CL._registerModule('grants/grants-rank');
})();
```
Registration string is **path-style** `'grants/grants-rank'` (matches `loader.js`
convention — logs `[CL] Module loaded: grants/grants-rank`). If `CL.grants.rank` needs a new
...
`<script src="modules/scorecard/scorecard-choropleth.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/grants/grants-rank.js"></script>
```
...
wc -l app/modules/grants/grants-rank.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initGrantModule\b|const +initGrantModule\b|let +initGrantModule\b|window\.initGrantModule\b|initGrantModule *= *function|initGrantModule *= *async|initGrantModule *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initGrantModule\b|const +initGrantModule\b|let +initGrantModule\b|window\.initGrantModule\b|initGrantModule *= *function|initGrantModule *= *async|initGrantModule *= *\(' app/modules/grants/grants-rank.js       # expected: the anchors present
grep -c '<script src="modules/grants/grants-rank.js"></script>' app/index.html  # 1
node --check app/modules/grants/grants-rank.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: grants/grants-rank`.
...
```
- Exercise the feature these functions drive: Grant scoring/ranking engine + scoring profiles.
- In DevTools confirm `typeof window.initGrantModule` → `'function'` and
  `typeof CL.grants.rank` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/grants/grants-rank.js
git checkout -- app/index.html
rm app/modules/grants/grants-rank.js
```
