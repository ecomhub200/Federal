# Stage A Conversion 06 — `app/modules/data/road-type-mapping.js` IIFE → ESM
...
- **Module:** `app/modules/data/road-type-mapping.js`
- **CL namespace:** `data/road-type-mapping`
- **Baseline size (IIFE era):** 139 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/road-type-mapping.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/road-type-mapping.js` passes (pre-state).
- [ ] `wc -l app/modules/data/road-type-mapping.js` snapshotted (expect ~139).
...
5. Keep `CL._registerModule('data/road-type-mapping');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/road-type-mapping.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/road-type-mapping.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/road-type-mapping`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/road-type-mapping.js
```
