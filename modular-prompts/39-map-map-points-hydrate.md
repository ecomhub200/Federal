# CC Modular Extraction Prompt 39 — `app/modules/map/map-points-hydrate.js`
...
# 1. Locate the block. Snapshot range: L28242-L28400 (feature: Map points hydration from matview/IndexedDB cache.)
grep -nE 'function +_hydrateMapPointsFromMatview\b|const +_hydrateMapPointsFromMatview\b|let +_hydrateMapPointsFromMatview\b|window\._hydrateMapPointsFromMatview\b|_hydrateMapPointsFromMatview *= *function|_hydrateMapPointsFromMatview *= *async|_hydrateMapPointsFromMatview *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=28242 && ($2+0)<=28400' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/map/map-points-hydrate.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/map/map-boundary.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L28242–L28400, ~159 lines). The exact
declarations are the `INDEX_MAP_part1.md` rows inside that range. Anchor
declarations (use to find the block):
- `_hydrateMapPointsFromMatview` (and the helpers between it and the next named decl)

Globals (module-private — move with the code):
- `_mapPointsInFlight`
- `_MAP_POINTS_CACHE_DB`
- `_MAP_POINTS_CACHE_STORE`
...
## §2 Where to put it
Create `app/modules/map/map-points-hydrate.js`:

```js
/**
 * CL map.pointsHydrate module
 *
 * Extracted from app/index.html (snapshot L28242-L28400) on 2026-05-15.
 ...
 *   - window._hydrateMapPointsFromMatview → CL.map.pointsHydrate._hydrateMapPointsFromMatview
 *
...
```
Registration string is **path-style** `'map/map-points-hydrate'` (matches `loader.js`
convention — logs `[CL] Module loaded: map/map-points-hydrate`). If `CL.map.pointsHydrate` needs a new
...
`<script src="modules/map/map-boundary.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/map/map-points-hydrate.js"></script>
```
...
wc -l app/modules/map/map-points-hydrate.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +_hydrateMapPointsFromMatview\b|const +_hydrateMapPointsFromMatview\b|let +_hydrateMapPointsFromMatview\b|window\._hydrateMapPointsFromMatview\b|_hydrateMapPointsFromMatview *= *function|_hydrateMapPointsFromMatview *= *async|_hydrateMapPointsFromMatview *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +_hydrateMapPointsFromMatview\b|const +_hydrateMapPointsFromMatview\b|let +_hydrateMapPointsFromMatview\b|window\._hydrateMapPointsFromMatview\b|_hydrateMapPointsFromMatview *= *function|_hydrateMapPointsFromMatview *= *async|_hydrateMapPointsFromMatview *= *\(' app/modules/map/map-points-hydrate.js       # expected: the anchors present
grep -c '<script src="modules/map/map-points-hydrate.js"></script>' app/index.html  # 1
node --check app/modules/map/map-points-hydrate.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: map/map-points-hydrate`.
...
```
- Exercise the feature these functions drive: Map points hydration from matview/IndexedDB cache.
- In DevTools confirm `typeof window._hydrateMapPointsFromMatview` → `'function'` and
  `typeof CL.map.pointsHydrate` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/map/map-points-hydrate.js
git checkout -- app/index.html
rm app/modules/map/map-points-hydrate.js
```
