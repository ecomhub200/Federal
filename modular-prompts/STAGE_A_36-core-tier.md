# Stage A Conversion 36 — `app/modules/core/tier.js` IIFE → ESM
...
- **Module:** `app/modules/core/tier.js`
- **CL namespace:** `core/tier`
- **Baseline size (IIFE era):** 390 lines (re-snapshot before editing)
...
- [ ] `app/modules/core/tier.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/core/tier.js` passes (pre-state).
- [ ] `wc -l app/modules/core/tier.js` snapshotted (expect ~390).
...
```js
import { /* tierUI members used */ } from '../upload/upload-tier-ui.js';
```
...
5. Keep `CL._registerModule('core/tier');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/core/tier.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/core/tier.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: core/tier`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/core/tier.js
```
