# CC Modular Extraction Prompt 40 — `app/modules/ai/ai-mode.js`
...
# 1. Locate the block. Snapshot range: L12100-L16557 (feature: MUTCD AI mode toggle + location dropdown + chat.)
grep -nE 'ai[A-Z]|[Aa]ssistant' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=12100 && ($2+0)<=16557' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/ai/ai-mode.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/map/map-points-hydrate.js"></script>' app/index.html   # expected: 1 match
```
...
> (L12100–L16557) falls entirely inside the `navigateTo` mega-function
...
## §2 Where to put it
Create `app/modules/ai/ai-mode.js`:

```js
/**
 * CL ai.mode module
 *
 * Extracted from app/index.html (snapshot L12100-L16557) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/40-ai-ai-mode.md.
 * Responsibility: MUTCD AI mode toggle + location dropdown + chat.
 *
 * Public API (back-compat dual exposure):
 *   - window.(ai mode fns — see INDEX_MAP part1) → CL.ai.mode.(ai mode fns — see INDEX_MAP part1)
 *
 * Depends on (must load before this file): `ai/context`, `core/constants`
 */
(function(){
  'use strict';
  ...
  window.CL = window.CL || {};
  CL.ai = CL.ai || {};
  // expose the extracted named declarations: window.<fn> = <fn>; CL.ai.<fn> = <fn>;
  CL._registerModule('ai/ai-mode');
})();
```
Registration string is **path-style** `'ai/ai-mode'` (matches `loader.js`
convention — logs `[CL] Module loaded: ai/ai-mode`). If `CL.ai.mode` needs a new
...
`<script src="modules/map/map-points-hydrate.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/ai/ai-mode.js"></script>
```
...
wc -l app/modules/ai/ai-mode.js                      # ≈ extracted + ~25 wrapper
grep -nE 'ai[A-Z]|[Aa]ssistant' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'ai[A-Z]|[Aa]ssistant' app/modules/ai/ai-mode.js       # expected: the anchors present
grep -c '<script src="modules/ai/ai-mode.js"></script>' app/index.html  # 1
node --check app/modules/ai/ai-mode.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: ai/ai-mode`.
...
```
- Exercise the feature these functions drive: MUTCD AI mode toggle + location dropdown + chat.
- In DevTools confirm `typeof window.ai` → `'function'` and
  `typeof CL.ai.mode` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/ai/ai-mode.js
git checkout -- app/index.html
rm app/modules/ai/ai-mode.js
```
