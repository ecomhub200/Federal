# Stage A Conversion 04 — `app/modules/utils/date-utils.js` IIFE → ESM
...
- **Module:** `app/modules/utils/date-utils.js`
- **CL namespace:** `utils/date-utils`
...
- [ ] `app/modules/utils/date-utils.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/utils/date-utils.js` passes (pre-state).
- [ ] `wc -l app/modules/utils/date-utils.js` snapshotted (expect ~34).
...
3. `export` every member previously assigned to `CL.utils.*` or
...
4. Keep transitional `CL.utils = CL.utils || {}; CL.utils.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('utils/date-utils');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/utils/date-utils.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/utils/date-utils.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: utils/date-utils`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/utils/date-utils.js
```
