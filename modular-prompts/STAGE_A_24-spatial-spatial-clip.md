# Stage A Conversion 24 — `app/modules/spatial/spatial-clip.js` IIFE → ESM
...
- **Module:** `app/modules/spatial/spatial-clip.js`
- **CL namespace:** `spatial/spatial-clip`
- **Baseline size (IIFE era):** 115 lines (re-snapshot before editing)
...
- [ ] `app/modules/spatial/spatial-clip.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/spatial/spatial-clip.js` passes (pre-state).
- [ ] `wc -l app/modules/spatial/spatial-clip.js` snapshotted (expect ~115).
...
5. Keep `CL._registerModule('spatial/spatial-clip');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/spatial/spatial-clip.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/spatial/spatial-clip.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: spatial/spatial-clip`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/spatial/spatial-clip.js
```
