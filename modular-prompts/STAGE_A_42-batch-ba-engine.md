# Stage A Conversion 42 — `app/modules/batch-ba/batch-ba-engine.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-engine.js`
- **CL namespace:** `batch-ba/engine`
- **Baseline size (IIFE era):** 477 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-engine.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-engine.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-engine.js` snapshotted (expect ~477).
...
5. Keep `CL._registerModule('batch-ba/engine');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-engine.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-engine.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/engine`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-engine.js
```
