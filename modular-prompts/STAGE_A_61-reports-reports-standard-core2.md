# Stage A Conversion 61 — `app/modules/reports/reports-standard-core2.js` IIFE → ESM
...
- **Module:** `app/modules/reports/reports-standard-core2.js`
- **CL namespace:** `reports/reports-standard-core2` (CL key `CL.reports.standardCore`)
- **Baseline size (IIFE era):** 339 lines (re-snapshot before editing)
- **Post-snapshot module** — depends on `reports-standard-core.js`
...
- [ ] `app/modules/reports/reports-standard-core2.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/reports/reports-standard-core2.js` passes (pre-state).
- [ ] `wc -l app/modules/reports/reports-standard-core2.js` snapshotted (expect ~339).
- [ ] `STAGE_A_60` (reports-standard-core) is applied on this branch first
...
One real static edge (was `CL.reports.standardCore.*` reads owned by the
`-core` base file):
```js
import { /* members read */ } from './reports-standard-core.js';  // was CL.reports.standardCore.*
```
...
5. Keep `CL._registerModule('reports/reports-standard-core2');` (load tracker).
...
- Add this module to `app/main.js` at its topo position (L2 — one-hop on
  `reports-standard-core`; see `STAGE_A_MAIN_ENTRY_DRAFT.js`, already
  listed). Do **not** touch `app/index.html` script tags in this prompt.

## §3 Post-flight
- [ ] `node --check app/modules/reports/reports-standard-core2.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/reports/reports-standard-core2.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: reports/reports-standard-core2`
      prints; the Standard Reports band renders.

## §4 Rollback
```bash
git checkout -- app/modules/reports/reports-standard-core2.js
```
