# CC Modular Extraction Prompt 38 — `app/modules/map/map-boundary.js`
...
test -f app/modules/map/map-boundary.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/map/map-render.js"></script>' app/index.html   # expected: 1 match
```
...
- `(boundary restore fns)` (and the helpers between it and the next named decl)
...
## §2 Where to put it
Create `app/modules/map/map-boundary.js`:
 ...
 *   - window.ensureTierBoundaryDisplayed → CL.map.boundary.ensureTierBoundaryDisplayed
 *   - window.(boundary restore fns) → CL.map.boundary.(boundary restore fns)
 *
 * Depends on (must load before this file): `map/map-render`, `spatial/federal-boundaries`
 */
(function(){
  'use strict';
  ...
  window.CL = window.CL || {};
  CL.map = CL.map || {};
  window.ensureTierBoundaryDisplayed = ensureTierBoundaryDisplayed; CL.map.ensureTierBoundaryDisplayed = ensureTierBoundaryDisplayed;
  CL._registerModule('map/map-boundary');
})();
```
Registration string is **path-style** `'map/map-boundary'` (matches `loader.js`
convention — logs `[CL] Module loaded: map/map-boundary`). If `CL.map.boundary` needs a new
...
`<script src="modules/map/map-render.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/map/map-boundary.js"></script>
```
...
wc -l app/modules/map/map-boundary.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +ensureTierBoundaryDisplayed\b|const +ensureTierBoundaryDisplayed\b|let +ensureTierBoundaryDisplayed\b|window\.ensureTierBoundaryDisplayed\b|ensureTierBoundaryDisplayed *= *function|ensureTierBoundaryDisplayed *= *async|ensureTierBoundaryDisplayed *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +ensureTierBoundaryDisplayed\b|const +ensureTierBoundaryDisplayed\b|let +ensureTierBoundaryDisplayed\b|window\.ensureTierBoundaryDisplayed\b|ensureTierBoundaryDisplayed *= *function|ensureTierBoundaryDisplayed *= *async|ensureTierBoundaryDisplayed *= *\(' app/modules/map/map-boundary.js       # expected: the anchors present
grep -c '<script src="modules/map/map-boundary.js"></script>' app/index.html  # 1
node --check app/modules/map/map-boundary.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: map/map-boundary`.
...
```
- Exercise the feature these functions drive: Boundary display + tier boundary restore.
- In DevTools confirm `typeof window.ensureTierBoundaryDisplayed` → `'function'` and
  `typeof CL.map.boundary` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/map/map-boundary.js
git checkout -- app/index.html
rm app/modules/map/map-boundary.js
```
