# Stage A Conversion 25 — `app/modules/spatial/aggregate-loader.js` IIFE → ESM
...
- **Module:** `app/modules/spatial/aggregate-loader.js`
- **CL namespace:** `spatial/aggregate-loader`
- **Baseline size (IIFE era):** 207 lines (re-snapshot before editing)
...
- [ ] `app/modules/spatial/aggregate-loader.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/spatial/aggregate-loader.js` passes (pre-state).
- [ ] `wc -l app/modules/spatial/aggregate-loader.js` snapshotted (expect ~207).
...
5. Keep `CL._registerModule('spatial/aggregate-loader');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/spatial/aggregate-loader.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/spatial/aggregate-loader.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: spatial/aggregate-loader`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/spatial/aggregate-loader.js
```
