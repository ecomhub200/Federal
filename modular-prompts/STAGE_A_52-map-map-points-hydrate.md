# Stage A Conversion 52 — `app/modules/map/map-points-hydrate.js` IIFE → ESM
...
- **Module:** `app/modules/map/map-points-hydrate.js`
- **CL namespace:** `map/map-points-hydrate`
- **Baseline size (IIFE era):** 154 lines (re-snapshot before editing)
...
- [ ] `app/modules/map/map-points-hydrate.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/map/map-points-hydrate.js` passes (pre-state).
- [ ] `wc -l app/modules/map/map-points-hydrate.js` snapshotted (expect ~154).
...
4. Keep transitional `CL.map = CL.map || {}; CL.map.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('map/map-points-hydrate');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/map/map-points-hydrate.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/map/map-points-hydrate.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: map/map-points-hydrate`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/map/map-points-hydrate.js
```
