# Stage A Conversion 54 — `app/modules/ai/ai-mode-toggle.js` IIFE → ESM
...
- **Module:** `app/modules/ai/ai-mode-toggle.js`
- **CL namespace:** `ai/ai-mode-toggle` (CL key `CL.ai.modeToggle`)
- **Baseline size (IIFE era):** 259 lines (re-snapshot before editing)
- **Post-snapshot module** (IIFE prompt 40b) — added after Session C's
  survey; folded into the Session K design docs.
...
- [ ] `app/modules/ai/ai-mode-toggle.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/ai/ai-mode-toggle.js` passes (pre-state).
- [ ] `wc -l app/modules/ai/ai-mode-toggle.js` snapshotted (expect ~259).
...
3. `export` every member previously assigned to `CL.ai.modeToggle.*` or
   `window.*` (public API). Leave module-private helpers
   (e.g. the `AI_MODE_STORAGE_KEY` const) unexported.

### Imports to ADD (top of file)
_None — leaf module (only self `CL.ai.modeToggle` / `CL.ai` reads)._ If
the post-01–46 re-scan finds new cross-module reads, add the matching
...
legacy comment block; **delete every other `window.*` line** (consumers
`import` instead):
```js
...
4. Keep transitional `CL.ai = CL.ai || {}; CL.ai.modeToggle = …;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('ai/ai-mode-toggle');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/ai/ai-mode-toggle.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/ai/ai-mode-toggle.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: ai/ai-mode-toggle`
      prints; the 4 onclick survivors (AI Mode toggle + header API-key
      manager) still fire.

## §4 Rollback
```bash
git checkout -- app/modules/ai/ai-mode-toggle.js
```
