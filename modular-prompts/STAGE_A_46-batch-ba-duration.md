# Stage A Conversion 46 — `app/modules/batch-ba/batch-ba-duration.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-duration.js`
- **CL namespace:** `batch-ba/duration`
- **Baseline size (IIFE era):** 545 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-duration.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-duration.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-duration.js` snapshotted (expect ~545).
...
5. Keep `CL._registerModule('batch-ba/duration');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-duration.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-duration.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_54-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/duration`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-duration.js
```
