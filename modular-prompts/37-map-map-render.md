# CC Modular Extraction Prompt 37 — `app/modules/map/map-render.js`
...
# 1. Locate the block. Snapshot range: L56001-L59000 (feature: Marker/cluster/heat rendering + popups.)
...
awk -F'|' 'NR>9 && ($2+0)>=56001 && ($2+0)<=59000' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/map/map-render.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/map/map-layers.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L56001–L59000, ~3000 lines). The exact
declarations are the `INDEX_MAP_part2.md` rows inside that range. Anchor
declarations (use to find the block):
- `(map render/cluster fns)` (and the helpers between it and the next named decl)
...
```js
/**
 * CL map.render module
 *
 * Extracted from app/index.html (snapshot L56001-L59000) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/37-map-map-render.md.
 * Responsibility: Marker/cluster/heat rendering + popups.
 *
 * Public API (back-compat dual exposure):
 *   - window.(map render/cluster fns) → CL.map.render.(map render/cluster fns)
 *
 * Depends on (must load before this file): `map/map-layers`
 */
(function(){
  'use strict';
...
```
Registration string is **path-style** `'map/map-render'` (matches `loader.js`
convention — logs `[CL] Module loaded: map/map-render`). If `CL.map.render` needs a new
...
`<script src="modules/map/map-layers.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/map/map-render.js                      # ≈ extracted + ~25 wrapper
grep -nE '[Mm]ap' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE '[Mm]ap' app/modules/map/map-render.js       # expected: the anchors present
grep -c '<script src="modules/map/map-render.js"></script>' app/index.html  # 1
node --check app/modules/map/map-render.js               # must pass
git diff --stat                 # ONLY app/index.html + the new module file
```
On app load the console must show: `[CL] Module loaded: map/map-render`.
...
```
- Exercise the feature these functions drive: Marker/cluster/heat rendering + popups.
- In DevTools confirm `typeof window.map` → `'function'` and
  `typeof CL.map.render` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/map/map-render.js
git checkout -- app/index.html
rm app/modules/map/map-render.js
```
