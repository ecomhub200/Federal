# Stage A Conversion 09 — `app/modules/data/tab-loaders.js` IIFE → ESM
...
- **Module:** `app/modules/data/tab-loaders.js`
- **CL namespace:** `data/tabLoaders`
- **Baseline size (IIFE era):** 189 lines (re-snapshot before editing)
...
- [ ] `app/modules/data/tab-loaders.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/data/tab-loaders.js` passes (pre-state).
- [ ] `wc -l app/modules/data/tab-loaders.js` snapshotted (expect ~189).
...
5. Keep `CL._registerModule('data/tabLoaders');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/data/tab-loaders.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/data/tab-loaders.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: data/tabLoaders`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/data/tab-loaders.js
```
