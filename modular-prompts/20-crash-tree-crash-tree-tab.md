# CC Modular Extraction Prompt 20 — `app/modules/crash-tree/crash-tree-tab.js`
...
# 1. Locate the block. Snapshot range: L105300-L109000 (feature: Crash Tree tab: tree construction + risk-factor analysis.)
grep -nE 'function +initCrashTreeTab\b|const +initCrashTreeTab\b|let +initCrashTreeTab\b|window\.initCrashTreeTab\b|initCrashTreeTab *= *function|initCrashTreeTab *= *async|initCrashTreeTab *= *\(' app/index.html
...
awk -F'|' 'NR>9 && ($2+0)>=105300 && ($2+0)<=109000' INDEX_MAP_part*.md
#    Cross-check: none of those names belong to an off-limits module in CLAUDE.md.

# 3. Target module must not exist yet
test -f app/modules/crash-tree/crash-tree-tab.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/analysis/analysis-tab.js"></script>' app/index.html   # expected: 1 match
```
...
BLK_END]** confirmed in §0 (snapshot L105300–L109000, ~3701 lines). The exact
declarations are the `INDEX_MAP_part3.md` rows inside that range. Anchor
declarations (use to find the block):
- `initCrashTreeTab` (and the helpers between it and the next named decl)
...
```js
/**
 * CL crashTree.tab module
 *
 * Extracted from app/index.html (snapshot L105300-L109000) on 2026-05-15.
 * Round X modular refactor — see modular-prompts/20-crash-tree-crash-tree-tab.md.
 * Responsibility: Crash Tree tab: tree construction + risk-factor analysis.
 *
 * Public API (back-compat dual exposure):
 *   - window.initCrashTreeTab → CL.crashTree.tab.initCrashTreeTab
 *
...
(function(){
  ...
  window.CL = window.CL || {};
  CL.crash-tree = CL.crash-tree || {};
  window.initCrashTreeTab = initCrashTreeTab; CL.crash-tree.initCrashTreeTab = initCrashTreeTab;
  CL._registerModule('crash-tree/crash-tree-tab');
})();
```
Registration string is **path-style** `'crash-tree/crash-tree-tab'` (matches `loader.js`
convention — logs `[CL] Module loaded: crash-tree/crash-tree-tab`). If `CL.crashTree.tab` needs a new
...
`<script src="modules/analysis/analysis-tab.js"></script>` (this places it in the **LATE** cluster
...
wc -l app/modules/crash-tree/crash-tree-tab.js                      # ≈ extracted + ~25 wrapper
grep -nE 'function +initCrashTreeTab\b|const +initCrashTreeTab\b|let +initCrashTreeTab\b|window\.initCrashTreeTab\b|initCrashTreeTab *= *function|initCrashTreeTab *= *async|initCrashTreeTab *= *\(' app/index.html                     # expected: 0 matches (anchors gone)
grep -nE 'function +initCrashTreeTab\b|const +initCrashTreeTab\b|let +initCrashTreeTab\b|window\.initCrashTreeTab\b|initCrashTreeTab *= *function|initCrashTreeTab *= *async|initCrashTreeTab *= *\(' app/modules/crash-tree/crash-tree-tab.js       # expected: the anchors present
...
```
On app load the console must show: `[CL] Module loaded: crash-tree/crash-tree-tab`.
...
```
- Exercise the feature these functions drive: Crash Tree tab: tree construction + risk-factor analysis.
- In DevTools confirm `typeof window.initCrashTreeTab` → `'function'` and
  `typeof CL.crashTree.tab` is defined.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/crash-tree/crash-tree-tab.js
git checkout -- app/index.html
rm app/modules/crash-tree/crash-tree-tab.js
```
