# Stage A Conversion 44 — `app/modules/batch-ba/batch-ba-results.js` IIFE → ESM
...
- **Module:** `app/modules/batch-ba/batch-ba-results.js`
- **CL namespace:** `batch-ba/results`
- **Baseline size (IIFE era):** 286 lines (re-snapshot before editing)
...
- [ ] `app/modules/batch-ba/batch-ba-results.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/batch-ba/batch-ba-results.js` passes (pre-state).
- [ ] `wc -l app/modules/batch-ba/batch-ba-results.js` snapshotted (expect ~286).
...
5. Keep `CL._registerModule('batch-ba/results');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/batch-ba/batch-ba-results.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/batch-ba/batch-ba-results.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: batch-ba/results`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/batch-ba/batch-ba-results.js
```
