# Stage A Conversion 14 — `app/modules/analysis/crash-profile.js` IIFE → ESM
...
- **Module:** `app/modules/analysis/crash-profile.js`
- **CL namespace:** `analysis/crash-profile`
- **Baseline size (IIFE era):** 378 lines (re-snapshot before editing)
...
- [ ] `app/modules/analysis/crash-profile.js` currently uses the IIFE wrapper `(function(){ … })();`.
- [ ] `node --check app/modules/analysis/crash-profile.js` passes (pre-state).
- [ ] `wc -l app/modules/analysis/crash-profile.js` snapshotted (expect ~378).
...
3. `export` every member previously assigned to `CL.analysis.*` or
...
4. Keep transitional `CL.analysis = CL.analysis || {}; CL.analysis.X = X;`
   writes + the `window.CL = window.CL || {};` guard (stripped in a later
   Stage A-cleanup round, NOT here).
5. Keep `CL._registerModule('analysis/crash-profile');` (load tracker).
...
## §3 Post-flight
- [ ] `node --check app/modules/analysis/crash-profile.js` passes (no stray IIFE brace / bad import).
- [ ] No new `export`/`import` beyond this prompt's lists.
- [ ] `git diff --stat` = ONLY `app/modules/analysis/crash-profile.js` (+ the `app/main.js` import line
...
- [ ] Full app verification is DEFERRED to `STAGE_A_62-cutover`
      ...
      `does not provide an export named`; `[CL] Module loaded: analysis/crash-profile`
      prints; any onclick this module owns still fires.

## §4 Rollback
```bash
git checkout -- app/modules/analysis/crash-profile.js
```
