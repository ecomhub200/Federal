# CC Modular Extraction Prompt 28 — `app/modules/grants/grants-ai.js`
...
test -f app/modules/grants/grants-ai.js && echo "ABORT: target exists" || echo "OK: target free"

# 4. Confirm load anchor still present
grep -n '<script src="modules/grants/grants-rank.js"></script>' app/index.html   # expected: 1 match
```
...
## §2 Where to put it
Create `app/modules/grants/grants-ai.js`:
...
```
Registration string is **path-style** `'grants/grants-ai'` (matches `loader.js`
convention — logs `[CL] Module loaded: grants/grants-ai`). If `CL.grants.ai` needs a new
...
`<script src="modules/grants/grants-rank.js"></script>` (this places it in the **LATE** cluster
with correct load order vs. its dependencies):

```html
<script src="modules/grants/grants-ai.js"></script>
```
...
```
On app load the console must show: `[CL] Module loaded: grants/grants-ai`.
...
## §7 Rollback
```bash
git diff --stat   # must show only app/index.html and app/modules/grants/grants-ai.js
git checkout -- app/index.html
rm app/modules/grants/grants-ai.js
```
