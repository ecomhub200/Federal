...
function _activeTier() {
    try {
        if (typeof window.jurisdictionContext !== 'undefined') {
            return window.jurisdictionContext.viewTier || 'county';
        }
    } catch (e) {}
    return 'county';
}
 ...
 */
export function hasPreloader(tabId) {
 ...
 */
export async function tryPreload(tabId) {
 ...
 */
export function getTabRows(tabId) {
    var tier = _activeTier();
    var value = _activeTierValue(tier);
    var key = _cacheKey(tabId, tier, value);
    if (_cache[key]) return _cache[key];
    try {
        return (typeof window.crashState !== 'undefined' && window.crashState && Array.isArray(window.crashState.sampleRows))
 ...
 */
export function tabServedBySupabase(tabId) {
    var tier = _activeTier();
    var value = _activeTierValue(tier);
