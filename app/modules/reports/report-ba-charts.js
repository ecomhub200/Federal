/**
 * CL reports.baCharts — extracted from app/index.html (name-anchored,
 * post-shift live L61481-L61755). navigateTo-split round, prompt 42c1(b) Part 2.
 * Responsibility: Before/After report tail — chart rendering, monthly trend
 * aggregation, detailed comparison table, key findings list, conclusions
 * paragraphs. Called by `displayBAResults` in `report-ba-run.js`.
 *
 * Reads shared inline globals via classic-script global lexical env
 * (NOT mirrored): `baState`, `COL`, `calcEPDO`, `Chart` (CDN browser-global).
 *
 * Public API mirrors (all 5 fns — called by report-ba-run.js' displayBAResults):
 *   createBACharts, calculateMonthlyTrend, displayBADetailedTable,
 *   displayBAFindings, displayBAConclusions
 */
(function () {
  'use strict';
  window.CL = window.CL || {};
  ...
  // ─── EXTRACTED CODE START (verbatim from index.html L61481-L61755) ───
// Create BA charts
...
function displayBAConclusions(r, treatmentName) {
  ...
  window.createBACharts = createBACharts;                  CL.reports.createBACharts = createBACharts;
  window.calculateMonthlyTrend = calculateMonthlyTrend;    CL.reports.calculateMonthlyTrend = calculateMonthlyTrend;
  window.displayBADetailedTable = displayBADetailedTable;  CL.reports.displayBADetailedTable = displayBADetailedTable;
  window.displayBAFindings = displayBAFindings;            CL.reports.displayBAFindings = displayBAFindings;
  window.displayBAConclusions = displayBAConclusions;      CL.reports.displayBAConclusions = displayBAConclusions;

  CL._registerModule('reports/report-ba-charts');
})();
