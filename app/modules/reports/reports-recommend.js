/**
 * CL reports.recommend — extracted from app/index.html (name-anchored,
 * live L61183-L61431). navigateTo-split round, prompt 42d(c).
 * Responsibility: Memo-style recommendation builders — collision-type
 * breakdown, severe-crash patterns, generic/safety/VRU recommendations,
 * year-over-year trend analysis. All emit docx `Paragraph`/`TextRun`.
 *
 * Reads shared inline globals via classic-script global lexical env
 * (NOT mirrored): `docx` (browser-global from CDN), `COL`, `MEMO_STYLES`.
 *
 * Public API mirrors (all 6 fns — inline callers L60751, L60846, L60851,
 * L60939, L60949, L61039, L61164):
...
(function () {
  ...
  // ─── EXTRACTED CODE START (verbatim from index.html L61183-L61431) ───
...
function generateTrendAnalysis(byYear, years) {
  ...
  window.buildCollisionTypeBreakdown = buildCollisionTypeBreakdown;          CL.reports.buildCollisionTypeBreakdown = buildCollisionTypeBreakdown;
  window.buildSevereCrashPatterns = buildSevereCrashPatterns;                CL.reports.buildSevereCrashPatterns = buildSevereCrashPatterns;
  window.generateMemoRecommendations = generateMemoRecommendations;          CL.reports.generateMemoRecommendations = generateMemoRecommendations;
  window.generateSafetyMemoRecommendations = generateSafetyMemoRecommendations;  CL.reports.generateSafetyMemoRecommendations = generateSafetyMemoRecommendations;
  window.generateVRURecommendations = generateVRURecommendations;            CL.reports.generateVRURecommendations = generateVRURecommendations;
  window.generateTrendAnalysis = generateTrendAnalysis;                      CL.reports.generateTrendAnalysis = generateTrendAnalysis;
