/**
 * CL reports.baEmail — extracted from app/index.html (name-anchored,
 * post-shift live L61929-L62355). navigateTo-split round, prompt 42c2(a).
 * Responsibility: Before/After email scheduling — modal opener, PDF
 * generation for email, delivery mode/frequency UI updaters, next-delivery
 * calculator. Plus async test-email helper (extra between anchors).
 *
 * Reads shared inline globals via classic-script global lexical env
 * (NOT mirrored): baState, notificationState, showToast, calcEPDO,
 * brevoApi, hideLoading, showLoading, etc.
 *
 * Public API mirrors (all 6 fns — many HTML onchange/onclick + inline callers):
 *   openBAEmailSchedule, generateBAPDFForEmail, testBAEmailNotification (async),
 *   updateBADeliveryModeUI, updateBAFrequencyUI, calculateBANextDelivery
 */
(function () {
  'use strict';
  window.CL = window.CL || {};
  ...
  // ─── EXTRACTED CODE START (verbatim from index.html L61929-L62355) ───
...
// ============================================================
  ...
  window.openBAEmailSchedule = openBAEmailSchedule;          CL.reports.openBAEmailSchedule = openBAEmailSchedule;
  window.generateBAPDFForEmail = generateBAPDFForEmail;      CL.reports.generateBAPDFForEmail = generateBAPDFForEmail;
  window.testBAEmailNotification = testBAEmailNotification;  CL.reports.testBAEmailNotification = testBAEmailNotification;
  window.updateBADeliveryModeUI = updateBADeliveryModeUI;    CL.reports.updateBADeliveryModeUI = updateBADeliveryModeUI;
  window.updateBAFrequencyUI = updateBAFrequencyUI;          CL.reports.updateBAFrequencyUI = updateBAFrequencyUI;
  window.calculateBANextDelivery = calculateBANextDelivery;  CL.reports.calculateBANextDelivery = calculateBANextDelivery;
