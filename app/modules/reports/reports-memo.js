/**
 * CL reports.memo — extracted from app/index.html (name-anchored,
 * live L60400-L60685). navigateTo-split round, prompt 42d(b).
 * Responsibility: Word-memo builders — MEMORANDUM header, severity stats
 * table, findings bullet list, top-locations table, footer block, and
 * full Document/Header/Footer factory.
 *
 * Reads shared inline globals via classic-script global lexical env
 * (NOT mirrored): `docx` (CDN browser-global), `COL`, `MEMO_STYLES`,
 * `calcEPDO`, `getDataSourceLabel`, `getFullTimestamp`.
 *
 * Public API mirrors (all 6 fns — many inline callers across L60709-L60952+):
 *   buildMemoHeader, buildMemoStatsTable, buildMemoFindings,
 *   buildMemoLocationsTable, buildMemoFooter,
 *   createWordDocumentWithHeaderFooter
 */
(function () {
  'use strict';
  window.CL = window.CL || {};
  ...
  // ─── EXTRACTED CODE START (verbatim from index.html L60400-L60685) ───
...
function createWordDocumentWithHeaderFooter(children, title) {
        ...
        children: [
            new Paragraph({
                ...
                children: [
                    ...
                    new TextRun({ text: 'Data: Virginia DMV', size: 14, color: '94A3B8', font: MEMO_STYLES.fontFamily }),
                    new TextRun({ children: [new Tab()] }),
  ...
  window.buildMemoHeader = buildMemoHeader;                                        CL.reports.buildMemoHeader = buildMemoHeader;
  window.buildMemoStatsTable = buildMemoStatsTable;                                CL.reports.buildMemoStatsTable = buildMemoStatsTable;
  window.buildMemoFindings = buildMemoFindings;                                    CL.reports.buildMemoFindings = buildMemoFindings;
  window.buildMemoLocationsTable = buildMemoLocationsTable;                        CL.reports.buildMemoLocationsTable = buildMemoLocationsTable;
  window.buildMemoFooter = buildMemoFooter;                                        CL.reports.buildMemoFooter = buildMemoFooter;
  window.createWordDocumentWithHeaderFooter = createWordDocumentWithHeaderFooter;  CL.reports.createWordDocumentWithHeaderFooter = createWordDocumentWithHeaderFooter;
