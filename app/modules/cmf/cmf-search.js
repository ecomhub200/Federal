 ...
 *
 * Extracted from app/index.html (name-anchored, live L80361-L80604)
 * on 2026-05-17. Round X modular refactor — see
 * modular-prompts/31-cmf-cmf-search.md.
 * Responsibility: CMF database load + location population + search.
 *
 * Moved fns (7): loadCMFDatabase, transformCMFData, showCMFLoadedStatus,
 *   initCMFLocationDropdown, updateCMFLocationDropdown, buildCMFSearchData,
 *   populateCMFLocations.
 *
 * Shared globals cmfState (inline L~76192), COL, crashState are NOT moved
 * and NOT mirrored — they stay inline and resolve via the shared
 * classic-script global lexical environment (proven pattern, e.g.
 * batch-ba/batch-ba-engine.js). This intentionally overrides the prompt's
 * "window-mirror cmfState" note (would be an out-of-block edit; unnecessary).
 *
 * Public API (back-compat dual exposure): window.<fn> + CL.cmf.<fn>
 * Depends on (must load before): core/constants, grants/grants-ui (load anchor).
 */
(function(){
  'use strict';
  // ─── EXTRACTED CODE START (verbatim from index.html L80361-L80604) ───
