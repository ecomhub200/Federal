/** CL pedbike.pedExport — extracted (name-anchored) 2026-05-23.
 *  see queue/203-passb-pedbike.md. No behavior change.
 *  Reads inline shared pedAnalysisState (window-mirrored). */
(function(){ 'use strict';
  // ─── EXTRACTED CODE START (verbatim) ───
/**
 * Export pedestrian crashes to KML
 */
function exportPedCrashesToKML() {
    const pedCrashes = crashState.sampleRows.filter(row =>
        row[COL.PED] === '1' || row[COL.PED] === 1 || row[COL.PED] === true || row[COL.PED] === 'Y' || row[COL.PED] === 'y' || row[COL.PED] === 'Yes' || row[COL.PED] === 'yes'
    );

    if (pedCrashes.length === 0) {
        alert('No pedestrian crashes found');
        return;
    }

    const title = `Pedestrian Crashes - ${pedCrashes.length} Total`;
    const kml = generateKML(pedCrashes, { title });
    downloadKML(kml, `pedestrian_crashes_${new Date().toISOString().split('T')[0]}`);
}

// ════════════════════════════════════════════════════════════
// PEDESTRIAN VIEW MODE AND EXPORT FUNCTIONS
// ════════════════════════════════════════════════════════════

function setPedViewMode(mode) {
    pedAnalysisState.viewMode = mode;
    document.getElementById('btnPedViewCombined').classList.toggle('active', mode === 'combined');
    document.getElementById('btnPedViewCompare').classList.toggle('active', mode === 'compare');
    updatePedDetailPanel();
}

function exportPedDetailCSV() {
    ...
    const rows = [
        ['Pedestrian Crash Detailed Analysis'],
    ...
    downloadFile(csv, `pedestrian_detail_${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
}

function exportPedDetailPDF() {
    const selected = pedAnalysisState.selectedLocations;
    if (!selected.length) { alert('No locations selected'); return; }

    showLoading('Generating Pedestrian Detailed Analysis PDF...');
    ...
    try {
        ...
        doc.setFontSize(12);
        doc.setFont(undefined, 'normal');
        doc.text('Pedestrian Crash Detailed Analysis', margin, 26);
        doc.setFontSize(11);
        ...
        doc.save(`pedestrian_analysis_${dateStamp}.pdf`);
        hideLoading();
    } catch (e) {
        hideLoading();
...
function exportPedDetailKML() {
    ...
    const title = `Pedestrian Analysis - ${selected.length} Locations, ${allCrashes.length} Crashes`;
    const description = `Locations: ${selected.map(s => s.location).join(', ')}`;
    const kml = generateKML(allCrashes, { title, description });
    downloadKML(kml, `pedestrian_detail_${new Date().toISOString().split('T')[0]}`);
}

function exportPedLocationsCSV() {
    const locations = pedAnalysisState.allLocations;
    if (!locations.length) { alert('Run analysis first'); return; }

    const rows = [
        ['Pedestrian High-Crash Location Analysis'],
    ...
    const csv = rows.map(r => r.map(c => `"${c}"`).join(',')).join('\n');
    downloadFile(csv, `pedestrian_locations_${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
}

function exportPedLocationsPDF() {
    const locations = pedAnalysisState.allLocations;
    if (!locations.length) { alert('Run analysis first'); return; }
    ...
    const themeColors = { primary: [8, 145, 178], secondary: [6, 182, 212] };
    const sevColors = SELECTION_PDF_STYLES.severityColors;
    const generatedDate = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const dateStamp = new Date().toISOString().split('T')[0];
    // Period must reflect the Pedestrian tab's own date filter (pedStartDate /
    // pedEndDate), not the full data range. Falls back to crashState.years.
    const yearRange = resolveReportPeriod('pedStartDate', 'pedEndDate');

    const ped = crashState.aggregates.ped;
    const pedEPDO = calcEPDO(ped);
    const kaCount = ped.K + ped.A;
    ...
    doc.setFontSize(12);
    doc.setFont(undefined, 'normal');
    doc.text('Pedestrian High-Crash Locations Report', margin, 26);
    doc.setFontSize(11);
    ...
    doc.text('Total Ped Crashes: ' + ped.total.toLocaleString(), pageWidth - margin - 62, 27);
    doc.text('Generated: ' + generatedDate, pageWidth - margin - 62, 34);
    ...
    // === KPI CARDS ===
    const kpiWidth = (contentWidth - 15) / 4;
    drawKPI(margin, y, kpiWidth, 'TOTAL PED CRASHES', ped.total.toLocaleString(), themeColors.primary);
    drawKPI(margin + kpiWidth + 5, y, kpiWidth, 'FATAL (K)', ped.K, [220, 53, 69]);
    drawKPI(margin + (kpiWidth + 5) * 2, y, kpiWidth, 'FATAL + SERIOUS', kaCount, [253, 126, 20]);
    drawKPI(margin + (kpiWidth + 5) * 3, y, kpiWidth, 'EPDO SCORE', pedEPDO.toLocaleString(), [124, 58, 237]);
    ...
    const sevOrder = [
        { code: 'K', count: ped.K, color: sevColors.K, label: 'Fatal' },
        { code: 'A', count: ped.A, color: sevColors.A, label: 'Serious' },
        { code: 'B', count: ped.B, color: sevColors.B, label: 'Minor' },
        { code: 'C', count: ped.C, color: sevColors.C, label: 'Possible' },
        { code: 'O', count: ped.O, color: sevColors.O, label: 'PDO' }
    ];
    let barX = margin;
    const barHeight = 8;
    sevOrder.forEach(s => {
        if (s.count > 0 && ped.total > 0) {
            const segWidth = (s.count / ped.total) * contentWidth;
    ...
    sevOrder.forEach(s => {
        if (s.count > 0) {
            ...
            doc.rect(legendX, y, 5, 3, 'F');
            doc.setTextColor(51, 51, 51);
            const pctStr = ped.total > 0 ? (s.count / ped.total * 100).toFixed(1) + '%' : '0%';
    ...
    doc.setTextColor(...themeColors.primary);
    doc.setFont(undefined, 'bold');
    doc.text('Ranked Pedestrian Crash Locations', margin, y + 5);
    doc.setFont(undefined, 'normal');
    doc.setDrawColor(...themeColors.secondary);
    doc.setLineWidth(0.7);
    doc.line(margin, y + 8, margin + contentWidth, y + 8);
    ...
    doc.save(`pedestrian_locations_${dateStamp}.pdf`);
}
  // ─── EXTRACTED CODE END ───
  window.CL=window.CL||{}; CL.pedbike=CL.pedbike||{};
  CL.pedbike.pedExport=CL.pedbike.pedExport||{};
  window.exportPedCrashesToKML=exportPedCrashesToKML; CL.pedbike.pedExport.exportPedCrashesToKML=exportPedCrashesToKML;
  window.setPedViewMode=setPedViewMode; CL.pedbike.pedExport.setPedViewMode=setPedViewMode;
  window.exportPedDetailCSV=exportPedDetailCSV; CL.pedbike.pedExport.exportPedDetailCSV=exportPedDetailCSV;
  window.exportPedDetailPDF=exportPedDetailPDF; CL.pedbike.pedExport.exportPedDetailPDF=exportPedDetailPDF;
  window.exportPedDetailKML=exportPedDetailKML; CL.pedbike.pedExport.exportPedDetailKML=exportPedDetailKML;
  window.exportPedLocationsCSV=exportPedLocationsCSV; CL.pedbike.pedExport.exportPedLocationsCSV=exportPedLocationsCSV;
  window.exportPedLocationsPDF=exportPedLocationsPDF; CL.pedbike.pedExport.exportPedLocationsPDF=exportPedLocationsPDF;
  CL._registerModule('pedbike/pedbike-tab-ped-export');
})();
